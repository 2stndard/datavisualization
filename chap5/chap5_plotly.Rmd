...
names(df_취업통계)[10:12] <- c('졸업자수', '취업률', '취업자수')
...
oecd <- covid19_stat |> filter(is.finite(백신접종완료률), 
                       is.finite(십만명당사망자수), 
                       !is.na(백신접종완료률), 
                       !is.na(십만명당사망자수), 
                       십만명당사망자수 != 0, 
...
nooecd <- covid19_stat |> filter(is.finite(백신접종완료률), 
                       is.finite(십만명당사망자수), 
                       !is.na(백신접종완료률), 
                       !is.na(십만명당사망자수), 
                       십만명당사망자수 != 0, 
...
korea <- covid19_stat |> filter(location == 'South Korea')
...
```{r eval = FALSE}
total_deaths_5_nations_by_day <- covid19_df |> 
...
```{r eval = FALSE}
total_deaths_5_nations <- covid19_df |> 
  filter((iso_code %in% c('KOR', 'USA', 'JPN', 'GBR', 'FRA'))) |>
  ...
  ## new_cases의 합계 산출
  summarise(total_deaths = max(total_deaths), population = max(population)) |>
  mutate(date = as.Date(yearmonth, format = '%Y %m'), 
         million_per_total_deaths = total_deaths / population * 100000) |>
  ungroup() 

total_deaths_5_nations |>
  ## plotly 객체 생성
  plot_ly() |>
  add_trace(type = 'scatter', mode = 'lines+text', 
            x = ~date, y = ~million_per_total_deaths , linetype = ~location, connectgaps = T)
...
```{r eval = FALSE}
p1 <- total_deaths_5_nations_by_day |>
  filter((iso_code %in% c('KOR'))) |>
  ## plotly 객체 생성
  plot_ly() |>
  add_trace(type = 'scatter', mode = 'lines+text', 
            x = ~date, y = ~total_deaths_per_million , linetype = ~location, connectgaps = T, 
            color = ~location, colors = RColorBrewer::brewer.pal(7, 'Blues')[7], fill = 'tonexty') |>
  layout(yaxis = list(title = '', range = c(0, 3000)))

p2 <- total_deaths_5_nations_by_day |>
  filter((iso_code %in% c('USA'))) |>
  ## plotly 객체 생성
  plot_ly() |>
  add_trace(type = 'scatter', mode = 'lines+text', 
            x = ~date, y = ~total_deaths_per_million , linetype = ~location, connectgaps = T, 
            color = ~location, colors = RColorBrewer::brewer.pal(7, 'Blues')[6], fill = 'tonexty') |>
  layout(yaxis = list(title = '', range = c(0, 3000)))

p3 <- total_deaths_5_nations_by_day |>
  filter((iso_code %in% c('JPN'))) |>
  ## plotly 객체 생성
  plot_ly() |>
  add_trace(type = 'scatter', mode = 'lines+text', 
            x = ~date, y = ~total_deaths_per_million , linetype = ~location, connectgaps = T, 
            color = ~location, colors = RColorBrewer::brewer.pal(7, 'Blues')[5], fill = 'tonexty') |>
  layout(yaxis = list(title = '10만명당 사망자수', range = c(0, 3000)))

p4 <- total_deaths_5_nations_by_day |>
  filter((iso_code %in% c('GBR'))) |>
  ## plotly 객체 생성
  plot_ly() |>
  add_trace(type = 'scatter', mode = 'lines+text', 
            x = ~date, y = ~total_deaths_per_million , linetype = ~location, connectgaps = T, 
            color = ~location, colors = RColorBrewer::brewer.pal(7, 'Blues')[4], fill = 'tonexty') |>
  layout(yaxis = list(title = '', range = c(0, 3000)))

p5 <- total_deaths_5_nations_by_day |>
  filter((iso_code %in% c('FRA'))) |>
  ## plotly 객체 생성
  plot_ly() |>
  add_trace(type = 'scatter', mode = 'lines+text', 
            x = ~date, y = ~total_deaths_per_million , linetype = ~location, connectgaps = T, 
            color = ~location, colors = RColorBrewer::brewer.pal(7, 'Blues')[3], fill = 'tonexty') |>
  layout(yaxis = list(title = '', range = c(0, 3000)))

subplot(p5, p4, p3, p2, p1, 
        nrows = 3, shareX = T, shareY = T, titleX = F, titleY = T 
        ) |>
...
total_deaths_5_nations |> 
  plot_ly() |>
  add_trace(type = 'scatter', mode = 'lines+text', 
            x = ~date, y = ~million_per_total_deaths , linetype = ~location,
            color = ~location, colors = RColorBrewer::brewer.pal(7, 'Blues')[3:7], 
            connectgaps = T) |>
  layout(xaxis = list(dtick = "M2", tickformat="%b\n%Y"))
...
total_deaths_5_nations |>
  ...
  add_trace(type = 'scatter', mode = 'lines+text', 
            x = ~date, y = ~million_per_total_deaths , linetype = ~location, connectgaps = T) |>
  add_annotations(text = '프랑스', 
                  x = total_deaths_5_nations |> filter(location == 'France', date == max(date)) |>
                      select(date) |> pull(), 
                  y = total_deaths_5_nations |> filter(location == 'France', date == max(date)) |>
                      select(million_per_total_deaths) |> pull(),
                  xanchor = 'left', showarrow = FALSE
                    ) |>
  add_annotations(text = '일본', 
                  x = total_deaths_5_nations |> filter(location == 'Japan', date == max(date)) |>
                      select(date) |> pull(), 
                  y = total_deaths_5_nations |> filter(location == 'Japan', date == max(date)) |>
                      select(million_per_total_deaths) |> pull(),
                  xanchor = 'left', yanchor = 'top', showarrow = FALSE
                    ) |>
  add_annotations(text = '영국', 
                  x = total_deaths_5_nations |> filter(location == 'United Kingdom', date == max(date)) |>
                      select(date) |> pull(), 
                  y = total_deaths_5_nations |> filter(location == 'United Kingdom', date == max(date)) |>
                      select(million_per_total_deaths) |> pull(),
                  xanchor = 'left', showarrow = FALSE
                    ) |>
  add_annotations(text = '미국', 
                  x = total_deaths_5_nations |> filter(location == 'United States', date == max(date)) |>
                      select(date) |> pull(), 
                  y = total_deaths_5_nations |> filter(location == 'United States', date == max(date)) |>
                      select(million_per_total_deaths) |> pull(),
                  xanchor = 'left', showarrow = FALSE
                    ) |>
  add_annotations(text = '한국', 
                  x = total_deaths_5_nations |> filter(location == 'South Korea', date == max(date)) |>
                      select(date) |> pull(), 
                  y = total_deaths_5_nations |> filter(location == 'South Korea', date == max(date)) |>
                      select(million_per_total_deaths) |> pull(),
                  xanchor = 'left', yanchor = 'bottom', showarrow = FALSE
                    ) |>
  layout(showlegend = FALSE)
...
total_deaths_5_nations_by_day |>
  ...
  layout(showlegend = T, margin = margins, 
...
total_deaths_5_nations_by_day |>
  ...
  layout(showlegend = T, margin = margins, 
         ...
         xaxis = list(range = c(min(total_deaths_5_nations_by_day$date),
...
total_deaths_5_nations_by_day |>
  ...
  layout(xaxis = list(range = c(min(total_deaths_5_nations_by_day$date),
                                max(total_deaths_5_nations_by_day$date))
                      ),
