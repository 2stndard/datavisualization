...
df_취업통계 |> filter(졸업자_계 < 500) |>
  ggplot(aes(x = 졸업자_계, y = 취업률_계)) +
  geom_point()

df_취업통계 |> filter(졸업자_계 < 500) |>
  ggplot(aes(x = 졸업자_계, y = 취업자_합계_계)) +
  geom_point(aes(color = 대계열))

df_취업통계 |> filter(졸업자_계 < 500) |>
  ggplot(aes(x = 졸업자_계, y = 취업자_합계_계)) +
  geom_point(aes(color = 대계열)) + 
  geom_smooth(aes(color = 대계열))
...
df_취업통계 |> filter(졸업자_계 < 500) |>
  ggplot(aes(x = 졸업자_계, y = 취업자_합계_계)) +
  geom_point(aes(color = 대계열)) + 
  geom_point(data = df_취업통계 |> filter(졸업자_계 < 500, 학과명 == '건축학과'), color = 'black', size = 3) +
  ggrepel::geom_text_repel(data = df_취업통계 |> filter(졸업자_계 < 500, 학과명 == '건축학과'), aes(label = 과정구분))
...
df_취업통계 |> filter(졸업자_계 < 500, 과정구분 == '대학과정') |>
  ggplot(aes(x = 졸업자_계, y = 취업자_합계_계)) +
  geom_point(aes(color = 대계열)) +
   geom_rug(col="steelblue",alpha=0.1, size=1.5)
...
df_취업통계 |> filter(졸업자_계 < 500) |>
  ...
  geom_point(aes(color = 대계열)) + 
    geom_point(data = df_취업통계 |> filter(졸업자_계 < 500, 학과명 == '건축학과'), color = 'black', size = 3) +
  ggrepel::geom_text_repel(data = df_취업통계 |> filter(졸업자_계 < 500, 학과명 == '건축학과'), aes(label = 과정구분)) +
  geom_encircle(data = df_취업통계 |> filter(졸업자_계 < 500, 학과명 == '건축학과'),  
                color="black", 
                size=2, 
                expand=0.1)
...
```{r}
df_취업통계 |> filter(졸업자_계 < 25, 대계열 == '인문계열') |>
  ggplot(aes(x = 졸업자_계, y = 취업률_계, color = 과정구분)) +
  geom_point()
...
p_scatter <- df_취업통계 |> filter(졸업자_계 < 500) |>
  ggplot(aes(x = 졸업자_계, y = 취업률_계)) +
  geom_point()


  ggMarginal(p_scatter, df_취업통계, y = 취업률_합계_계, type = 'density', color="purple", size=4)
