...
표는 행과 열로 이루어진 데이터의 모음이다. 행은 각각의 개별 사례2나 개별 사례를 적절히 그룹화한 요약된 데이터가 표현된다. 열은 각각의 사례를 설명하기 위해 필요한 속성이 나열된다. 행과 열은 이 만나는 곳이 사용자가 알고 싶어하는 데이터가 위치하고 이 곳을 칸, 혹은 셀이라고 한다.
...
![그림 4-1 1차원 표의 예 (출처 : https://docs.tibco.com/pub/spotfire/6.5.2/doc/html/images/cross_example_table.png)](C:/R/git/datavisualization/fig/4-1.png)

![그림 4-2 2차원 표의 예 (출처 : https://docs.tibco.com/pub/spotfire/6.5.2/doc/html/images/cross_example_cross_table.png)](C:/R/git/datavisualization/fig/4-2.png)
...
![그림 4-3 미국 심리학회 표 구성 (출처 : https://libapps.s3.amazonaws.com/customers/836/qu/e1d262992c7ea1e2aa109305faf24d59.png)](C:/R/git/datavisualization/fig/4-3.png)
...
![그림 4-4 gt 표의 구성 (출처 : https://gt.rstudio.com/reference/figures/gt_parts_of_a_table.svg)](C:/R/git/datavisualization/fig/4-4.png)
...
![그림 4-5 gt 패키지의 워크플로 (출처 : https://gt.rstudio.com/reference/figures/gt_workflow_diagram.svg)](C:/R/git/datavisualization/fig/4-5.png)
...
gt(data, rowname_col = "rowname", groupname_col = dplyr::group_vars(data), caption = NULL, rownames_to_stub = FALSE, auto_align = TRUE, id = NULL, row_group.sep = getOption("gt.row_group.sep", " - "))  
* data : 표에 사용할 데이터프레임이나 tibble  
* rowname_col : 표 스텁으로 사용할 열 이름  
* groupname_col : 행 그룹으로 그룹화할 열 이름  
* caption : R 마크다운에서 사용되는 추가적인 캡션 설정  
* rownames_to_stub : 입력 데이터 테이블에서 행 캡션을 표 스텁에 표시하는 설정  
* auto_align : 각각의 셀 데이터의 특성에 따라 자동 정렬 설정  
* id : 표 ID  
* row_group.sep : 표시된 스텁 행 그룹 이름에서 연속된 그룹 이름 사이에 사용할 구분 기호  

:::

매개변수 없이 `gt()`만으로도 `gt` 객체가 생성된다. 이 경우는 앞서 설명한 1차원 표의 형태인 단순 표(simple table)의 형태이다. 하지만 2차원 표의 형태를 만들기 위해서는 행에 대한 이름(Stub)을 붙여주고 유사한 행들을 그룹화(row group)하여야 한다. 행 그룹으로 사용할 열은 `groupname_col`로, 표의 스텁(Stub)으로 사용할 열인 `rowname_col`을 설정해주면 표의 전반적인 기본 형태를 볼 수 있기 때문에 가급적이면 `gt` 객체를 생성할 때 설정해 주는 것이 좋다.
...
![그림 4-6 기본 gt 표](C:/R/git/datavisualization/fig/4-6.emf)
...
![그림 4-7 표제가 설정된 gt 표](C:/R/git/datavisualization/fig/4-7.emf)
...
![그림 4-8 데이터 포맷이 설정된 gt 표](C:/R/git/datavisualization/fig/4-8.emf)
...
![그림 4-9 스패너가 설정된 gt 표](C:/R/git/datavisualization/fig/4-9.emf)

앞선 전처리 과정에서 '1인창(사)업자' 열은 열이름의 맨 앞이 숫자이고 특수문자가 들어가기 때문에 열 이름을 '일인창사업자로' 설정했다. '1인창(사)업자'와 같이 숫자가 맨앞에 들어가는 열은 '\`'을 사용하여 접근이 가능하지만 여러모로 불편한점이 있다. 하지만 표에서의 표기는 원상태대로 해주겠다. `col_label()`을 이용하여 헤더에 표함된 열 이름을 바꾸어 줄 수 있다.

::: {custom-style="comment"}
cols_label(.data, ..., .list = list2(...))  
  - data : 표에 사용할 gt 객체  
  - ... : 열 이름 설정에 사용할 하나 이상의 매개변수  
  - list : ...대신 열 이름 설정에 사용할 리스트  
...
![그림 4-10 열 이름이 변경된 gt 표](C:/R/git/datavisualization/fig/4-10.emf)

# 요약 통계표를 만들자 - 행 그룹과 요약행 생성
...
![그림 4-11 행 그룹 순서가 변경된 gt 표](C:/R/git/datavisualization/fig/4-11.emf)
...
![그림 4-12 소계가 설정된 gt 표](C:/R/git/datavisualization/fig/4-12.emf)
...
![그림 4-13 총계가 설정된 gt 표](C:/R/git/datavisualization/fig/4-13.emf)

# 요약 통계표를 꾸미자 - 표 옵션 설정과 색을 통한 데이터 강조
...
```{r}
 gt_table9 <- gt_table8 |>
   tab_options(
     ...
     ## 구분(Stub) 행의 배경색 설정('#00688B')
     stub.background.color = '#00688B',
     ## 행 그룹 이름 표현 셀 배경색 설정('#00688B')
     row_group.background.color = '#00688B',
     ## 열 제목(Heading) 배경색 설정('#00688B')
     heading.background.color = '#00688B',
...
![그림 4-14 전체 표 옵션이 설정된 gt 표](C:/R/git/datavisualization/fig/4-14.emf)
...
![그림 4-15 개별 셀이 설정된 gt 표](C:/R/git/datavisualization/fig/4-15.emf)
...
![그림 4-16 헤드 속성이 설정된 gt 표](C:/R/git/datavisualization/fig/4-16.emf)
...
![그림 4-17 요약 행 속성이 설정된 gt 표](C:/R/git/datavisualization/fig/4-17.emf)
...
![그림 4-18 열 너비가 설정된 gt 표](C:/R/git/datavisualization/fig/4-18.emf)
...
![그림 4-19 열 색상이 설정된 gt 표](C:/R/git/datavisualization/fig/4-19.emf)

# 표에 그래프 넣기 - gtExtras를 사용한 그래프 넣기
...
![그림 4-20 패키지 업데이트 화면](C:/R/git/datavisualization/fig/4-20.png)
...
+-----------------------+----------------------------------------------------------------------------------------------------------------+
: 표 4 - 7 gtExtras 패키지 제공 테마

![그림 4-21 gtExtras에서 제공하는 gt 테마](C:/R/git/datavisualization/fig/4-21.png)
...
![그림 4-22 gtExtras에서 제공하는 그래프](C:/R/git/datavisualization/fig/4-22.png)
...
![그림 4-23 막대그래프가 추가된 gt 표](C:/R/git/datavisualization/fig/4-23.emf)
...
![그림 4-24 컬러 박스가 추가된 gt 표](C:/R/git/datavisualization/fig/4-24.emf)

# 비율과 값을 가지는 열 만들기 - 표의 열 병합
...
![그림 4-25 열이 병합된 gt 표](C:/R/git/datavisualization/fig/4-25.emf)
...
![그림 4-26 열이 복제된 gt 표](C:/R/git/datavisualization/fig/4-26.emf)
...
![그림 4-27 열이 병합된 최종 gt 표](C:/R/git/datavisualization/fig/4-27.emf)
