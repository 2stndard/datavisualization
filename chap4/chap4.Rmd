...
표의 헤드는 표의 데이터가 표시되어 데이터 각 열의 이름이 표시되는 줄이다. 표 헤드는 한 줄로 표현이 가능하지만 유사한 열끼리 묶어 여러 행으로 표현하는 경우도 있다. 이 부분을 스패너(Spanner)라고 한다. 결국 표 헤드를 설정한다는 것은 각각의 열 이름을 설정하고 유사한 열끼리를 묶어 스패너를 설정하는 과정이다. 표 헤드를 설정하기 위해서는 다음과 같은 함수들이 사용된다. 
