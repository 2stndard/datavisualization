...
![<https://d1zx6djv3kb1v7.cloudfront.net/wp-content/media/2019/09/What-do-you-mean-by-the-terms-Skewed-Data-Outliers-Missing-Values-and-Null-Values-1-i2tutorials.jpg>](skew.jfif)
...
박스플롯은 대부분이 X축은 이산형 데이터, Y축은 연속형 수치 데이터로 구성된다. X축이 이산형 데이터이기 때문에 추가적인 변수를 매핑함으로써 이산형 데이터를 다시 세분화할 수 있다. 이렇게 단계적으로 구성된 이산형 데이터는 하위 단계까지 그루핑해서 박스플롯을 만들수 있다. 이런 그루핑 박스플롯은 특별한 함수를 사용하는 것이 아니고 color나 fill로 추가 변수를 매핑함으로써 만들수 있다. 다음은 앞서 만든 박스플롯을 '과정구분' 변수로 세부 그룹화하여 만든 박스플롯이다.

```{r fig.cap='그루핑된 박스플롯'}

p_boxplot_highlight +
  geom_boxplot(aes(x = 대계열, y = 취업률_계, fill = 과정구분)) +
  labs(title = '그루핑된 박스플롯')

```

앞의 박스플롯은 색이 전반적으로 각 그룹별로 설정되어 있기 때문에 특정 계열의 강조가 사라졌다. 그루핑한 박스플롯에 특정 계열을 강조하기 위해서는 다음과 같이 사용할 수 있다.

```{r fig.cap='그루핑되고 강조된 박스플롯'}
p_boxplot_highlight +
  ...
  labs(title = '그루핑되고 강조된 박스플롯')
```

## facet boxplot

앞의 그루핑된 박스플롯은 그루핑된 변수의 변량이 적을때는 효과적으로 보이지만 변량이 많아져서 박스플롯이 많아지면 효과적인 시각화가 되지 않는다. 이럴 경우는 `facet_*()`을 사용하여 플롯을 분리시켜주는게 효과적이다.
...
p_boxplot +
  ...
  ## alpha를 0.2, color를 'orange'로 설정한 geom_jitter 레이어 생성 
  geom_point(aes(x = 대계열, y = 취업률_계), stat = 'summary', fun.y = 'mean', color = 'tomato3', shape = 4) + 
  geom_text(aes(x = 대계열, y = 취업률_계, label = round(..y.., 1)), stat = 'summary', fun.y = 'mean', color = 'tomato3', vjust = 1.5) + 
...
피라미드 플롯은 주로 인구의 분포를 시각화는데 많이 사용된다. 아마도 아래의 그림과 같은 플롯을 한번쯤은 본적이 있을 것이다. ![https://sgis.kostat.go.kr/jsp/pyramid/pyramid1.jsp](pyramid.png)
