...
```{r eval = FALSE}
total_deaths_5_natinos_by_day <- covid19_df |> 
  filter((iso_code %in% c('KOR', 'USA', 'JPN', 'GBR', 'FRA')))

total_deaths_5_natinos_by_day |>
  ## plotly 객체 생성
  plot_ly() |>
...
```{r eval = FALSE}
total_deaths_5_natinos |>
  ## plotly 객체 생성
  plot_ly() |>
  add_trace(type = 'scatter', mode = 'lines+text', 
            x = ~date, y = ~million_per_total_deaths , linetype = ~location, connectgaps = T) |>
  add_annotations(text = '프랑스', 
                  x = total_deaths_5_natinos |> filter(location == 'France', date == max(date)) |>
                      select(date) |> pull(), 
                  y = total_deaths_5_natinos |> filter(location == 'France', date == max(date)) |>
                      select(million_per_total_deaths) |> pull(),
                  xanchor = 'left', showarrow = FALSE
                    ) |>
  add_annotations(text = '일본', 
                  x = total_deaths_5_natinos |> filter(location == 'Japan', date == max(date)) |>
                      select(date) |> pull(), 
                  y = total_deaths_5_natinos |> filter(location == 'Japan', date == max(date)) |>
                      select(million_per_total_deaths) |> pull(),
                  xanchor = 'left', yanchor = 'top', showarrow = FALSE
                    ) |>
  add_annotations(text = '영국', 
                  x = total_deaths_5_natinos |> filter(location == 'United Kingdom', date == max(date)) |>
                      select(date) |> pull(), 
                  y = total_deaths_5_natinos |> filter(location == 'United Kingdom', date == max(date)) |>
                      select(million_per_total_deaths) |> pull(),
                  xanchor = 'left', showarrow = FALSE
                    ) |>
  add_annotations(text = '미국', 
                  x = total_deaths_5_natinos |> filter(location == 'United States', date == max(date)) |>
                      select(date) |> pull(), 
                  y = total_deaths_5_natinos |> filter(location == 'United States', date == max(date)) |>
                      select(million_per_total_deaths) |> pull(),
                  xanchor = 'left', showarrow = FALSE
                    ) |>
  add_annotations(text = '한국', 
                  x = total_deaths_5_natinos |> filter(location == 'South Korea', date == max(date)) |>
                      select(date) |> pull(), 
                  y = total_deaths_5_natinos |> filter(location == 'South Korea', date == max(date)) |>
                      select(million_per_total_deaths) |> pull(),
...
```{r eval = FALSE}
total_deaths_5_natinos <- covid19_df |> 
  filter((iso_code %in% c('KOR', 'USA', 'JPN', 'GBR', 'FRA'))) |>
