...
![](cross_example_table.png)

![](cross_example_cross_table.png)
...
![](e1d262992c7ea1e2aa109305faf24d59.png)
...
```{r data_import, warning=FALSE, message=FALSE, echo=FALSE}
library(openxlsx)
library(tidyverse)

## `read.xlsx()`을 사용하여 엑셀 파일의 데이터를 불러옴
df <- read.xlsx(xlsxFile = 'C:/R/git/datavisualization/data/21년 고등 학과별 입학정원 입학 지원 재적 재학 휴학 외국인유학생 졸업 교원_211119.xlsx', 
...
```{r data_import1, warning=FALSE, message=FALSE, eval = FALSE}
library(openxlsx)
library(tidyverse)
...
```{r gt_preview, message=FALSE, warning=FALSE, echo = FALSE}
library(gt)
df.gt |> 
  gt() |>
...
```{r gt_preview1, message=FALSE, warning=FALSE, echo = FALSE}
gt.table1 <- df.gt |> 
  gt(rowname_col = '학제', 
     groupname_col = '학위과정') |>
  tab_options(table.font.size = 10)
...
```{r gt_preview2, message=FALSE, warning=FALSE, echo = FALSE}
gt.table2 <- gt.table1 |> 
  tab_header(title = '고등교육기관 데이터', subtitle = '2021년 전체 고등교육기관 대상') |>
  tab_options(table.font.size = 10)
...
```{r gt_preview4, message=FALSE, warning=FALSE, echo = FALSE}
gt.table3 <- gt.table2 |> 
  tab_spanner(columns = 3:8, label = '합계') |>
  tab_spanner(columns = 9:14, label = '평균') |>
  tab_options(table.font.size = 10)
...
```{r gt_preview3, message=FALSE, warning=FALSE, echo = FALSE}
gt.table4 <- gt.table3 |> 
  fmt_number(columns = 3:8, decimals = 0, use_seps = TRUE) |>
  fmt_number(columns = 9:14, decimals = 1, use_seps = TRUE) |>
  tab_options(table.font.size = 10)
...
```{r gt_preview5, message=FALSE, warning=FALSE, echo = FALSE}
gt.table5 <- gt.table4 |> 
  cols_label(학제 = '학교종류', 
               학과수_전체_sum = '학과수', 
               지원자_전체_계_sum = '지원자',
...
```{r gt_preview6, message=FALSE, warning=FALSE, echo = FALSE}
gt.table6 <- gt.table5 |> 
  row_group_order(
...
```{r gt_preview7, message=FALSE, warning=FALSE, echo = FALSE}
gt.table7 <- gt.table6 |> 
  summary_rows(
    groups = T,
    columns = 3:8,
