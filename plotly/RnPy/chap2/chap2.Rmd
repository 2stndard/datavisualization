...
데이터를 시각화할 때 하나의 시각화에 여러 개의 데이터를 표시하는 경우는 매우 빈번하게 발생한다. 보통 이런 경우 각각의 데이터를 구별하기 위해 색상, 크기, 라인타입 등의  다양한 속성 값을 사용하여 데이터를 구분하지만 구분되어야 하는 데이터가 많은 경우는 데이터 구분 속성을 알아보기 힘들거나 일부 구간에 중복되어 알아보기 힘든 경우도 많다. 이런 경우 각각의 데이터들을 서브 플롯화하여 따로 떼서 작은 시각화를 만들어 주면 이런 현상을 해소할 수 있다. `plolty`에서도 이렇게 여러개의 서브 플롯을 만드는 기능을 지원하는데 동일한 트레이스를 사용하는 플롯으로 구성할 수도 있고 서브 플롯마다 각자의 트레이스를 사용할 수도 있다. 

## 서브 플롯 생성

`plolty`에서 서브 플롯을 사용하기 위해서 제공하는 함수는 R에서는  `subplot()`, python에서는 `make_subplot()`이다. 앞서 data와 layout의 경우 R과 python이 유사한 방법을 사용했지만 서브 플롯에서는 두 언어간 구현 방식이 다소 다르다. 

-   R

R에서 `plotly`가 구현되는 방식은 먼저 서브 플롯에 사용될 `plotly` 객체를 생성하고 이 객체들을 `subplot()`을 사용하여 서브 플롯을 구성한다. `subplot()`에서는 `plotly`객체 뿐 아니라 `ggplot2`로 만들어진 객체도 서브 플롯으로 구성할 수 있다. 또 `subplot()`에서 사용되는 매개변수들은 앞서 `add_trace()`나 `layout()`과 같이 JSON 데이터로 구조화한 속성이 아닌 함수의 매개변수 형태로 사용한다.  
...
`subplot()`의 사용시에 하나 주의해야할 부분은 서브 플롯은 서브 플롯으로 표현되는 플롯과 전체 플롯으로 구성된다는 것이다. 따라서 `layout()`을 사용하여 플롯의 세부 속성들을 설정할 때 전체 플롯으로 설정되는 것이 우선된다는 것이다. 예를 들어 플롯의 제목의 경우 세부 서브 플롯들의 제목과 전체 플롯의 제목이 모두 `layout()`의 'title' 속성으로 설정된다. 하지만 서브 플롯들에 `layout()`의 'title'로 세부 설정을 한다고 해도 결국은 전체 제목으로 설정된다는 것이다. 하지만 'xaxis'나 'yaxis'와 같이 전체 플롯에는 없는 설정의 경우는 각각의 세부 플롯에서 설정된 값들이 유지된다는 것이다. 

다음은 대륙별 확진자수의 선 그래프를 서브 플롯으로 구성하는 코드이다. 먼저 전체적인 `plotly` 객체를 초기화하고 이 초기화된 객체를 사용하여 한국, 아시아, 유럽, 북미, 남미, 오세아니아, 아프리카의 `plotly` 객체를 생성한다. 이 객체들을 `subplot()`을 사용하여 서브 플롯을 만들어주고  
...
p1 <- p_line_wide |> 
  add_trace(type = 'scatter', mode = 'lines', x = ~date, 
            y = ~확진자_한국, color = I(RColorBrewer::brewer.pal(7, 'Blues')[7]), name = '한국') |> 
  layout(title = list(title = '한국'),
...
p2 <- p_line_wide |> 
  add_trace(type = 'scatter', mode = 'lines', x = ~date, 
            y = ~확진자_아시아, color = I(RColorBrewer::brewer.pal(7, 'Blues')[6]), name = '아시아') |> 
  layout(title = list(title = '아시아'),
         xaxis = list(tickfont = list(size = 10)), 
         yaxis = list(title = list(text = '확진자수')))
...
p3 <- p_line_wide  |> 
  add_trace(type = 'scatter', mode = 'lines', x = ~date, 
            y = ~확진자_유럽, color = I(RColorBrewer::brewer.pal(7, 'Blues')[5]), name = '유럽') |> 
  layout(title = list(title = '유럽'),
         xaxis = list(tickfont = list(size = 10)), 
         yaxis = list(title = list(text = '확진자수')))
...
p4 <- p_line_wide  |> 
  add_trace(type = 'scatter', mode = 'lines', x = ~date, 
            y = ~확진자_북미, color = I(RColorBrewer::brewer.pal(7, 'Blues')[4]), name = '북미') |> 
  layout(title = list(title = '북미'),
         xaxis = list(tickfont = list(size = 10)), 
         yaxis = list(title = list(text = '확진자수')))
...
p5 <- p_line_wide  |> 
  add_trace(type = 'scatter', mode = 'lines', x = ~date, 
            y = ~확진자_남미, color = I(RColorBrewer::brewer.pal(7, 'Blues')[3]), name = '남미') |> 
  layout(title = list(title = '남미'),
         xaxis = list(tickfont = list(size = 10)), 
         yaxis = list(title = list(text = '확진자수')))
...
p6 <- p_line_wide  |> 
  add_trace(type = 'scatter', mode = 'lines', x = ~date,
            y = ~확진자_오세아니아, color = I(RColorBrewer::brewer.pal(7, 'Blues')[2]), name = '오세아니아') |> 
  layout(title = list(title = '오세아니아'),
         xaxis = list(tickfont = list(size = 10)), 
         yaxis = list(title = list(text = '확진자수')))
...
p7 <- p_line_wide  |> 
  add_trace(type = 'scatter', mode = 'lines', x = ~date, 
            y = ~확진자_아프리카, color = I(RColorBrewer::brewer.pal(7, 'Blues')[1]), name = '아프리카') |> 
  layout(title = list(title = '아프리카'),
         xaxis = list(tickfont = list(size = 10)), 
         yaxis = list(title = list(text = '확진자수')))
...
다만 앞선 절에서도 언급했다시피 python에서 `plotly`에서 간편하게 시각화를 만드는 방법으로 제공하는 plotly.express에서는 서브 플롯 기능을 제공하지 않고 패싯(facet) 속성을 사용해 서브 플롯과 유사한 기능을 제공한다. 
