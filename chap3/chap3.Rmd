...
`size` 는 기하요소의 크기를 설정한다. 기하요소가 점이면 점의 크기, 선이면 선의 굵기를 설정한다. 점의 크기를 결정할 때는 반지름의 길이를 밀리미터 단위로 지정한다. 또 선의 굵기도 밀리미터 단위로 설정할 수 있다.
...
```{r fig.cap='미적요소가 설정된 geom_histogram'}
## p_histogram에 x축을 '취업률_계'열로 매핑, 각각의 미적요소을 설정한  geom_histogram 레이어를 생성 
p_histogram + 
...
```{r fig.cap='미적요소를 설정한 geom_freqpoly'}
...
```{r fig.cap='group과 미적요소를 매핑한 geom_freqpoly'}
...
```{r fig.cap='미적요소가 설정된 geom_density'}
...
```{r fig.cap='미적요소가 설정된 geom_boxplot'}
## p_boxplot에 x축에 대계열, y축에 취업률_계, fill을 대계열로 매핑, 미적요소를 설정한 geom_boxplot레이어를 생성
p_boxplot + 
...
`geom_rect()`는 `ggplot` 객체에 사각형을 그리는 레이어를 생성하는 함수이다. 사각형을 그리기 위해 사각형의 네 꼭지점 좌표를 결정하기 위한 `xmin`, `ymin`, `xmax`, `ymax`값이 필요하고 사각형 선의 색이나 내부 색 등을 설정할 수 있다.
...
위의 코드는 앞선 코드와는 많이 다른 점이 있다. 우선 `geom_line()` 레이어를 생성하기 이전에 `geom_rect()`를 사용하여 강조 부분에 대한 레이어를 먼저 생성하였다. 앞서 설명한 바와 같이 `geom_rect()`에서 `alpha`가 정상적으로 작동하지 않는 문제 때문이다. 데이터가 매핑된 레이어가 생성되기 전에 생성된 `geom_rect()` 레이어에서는 `alpha`에 따른 투명도가 정상적으로 작동하지만 이미 생성된 레이어 위에 `geom_rect()`를 생성할 떄는 `alpha`가 정상적으로 작동하지 않는다.

#### geom_segment()

`geom_segment()`는 선으로 표현되는 미적요소에 대한 레이어를 생성한다. `geom_line()`과 유사하지만 데이터 매핑없이 사용한다는 점에서 주로 설명선을 그릴때 사용되는 레이어이다.

```{r eval=FALSE}
geom_segment(mapping = NULL, data = NULL, stat = "identity", position = "identity", arrow = NULL, arrow.fill = NULL, lineend = "butt", linejoin = "round", na.rm = FALSE, show.legend = NA, ...)
  - mapping : aes()를 사용하여 매핑할 미적요소, 생략되면 ggplot()에 정의된 미적매핑 사용
  - data : 시각화를 위해 사용될 데이터, 생략되면 ggplot()에 정의된 데이터 사용
  - stat : 시각화에 적용될 통계요소, 기본값은 'identity'
  - position : 시각화에 적용될 위치요소, 기본값은 'identity'
...
`geom_vline()`과 `geom_hline()`은 수직선과 수평선으로 표현되는 미적요소에 대한 레이어를 생성한다. `geom_segment()`과 유사하지만 `xintercept`나 `yintercept`의 하나의 매개변수만으로 수직선, 수평선을 생성할 수 있다. 주로 기준선을 설정할 때 사용한다.
...
`geom_vline()`과 `geom_hline()`에서 사용하는 미적요소는 `geom_segment()`와 동일하다.
...
`geom_abline()`은 대각선을 그릴때 사용하는 레이어이다. `geom_line()`도 대각선을 그릴수 있지만 `geom_line()`은 두 점을 이어주는 선을 그리는 반면 `geom_abline()`은 일차방정식 형태의 기울기와 y절편값을 사용하여 대각선을 그려준다. 당연히 기울기가 0이면 수평선이 그려지고 기울기가 무한대 이면 수직선이 그려진다.
...
`geom_abline()`에서 사용하는 미적요소는 `geom_segment()`와 동일하다.
...
p_boxplot + 
  ...
  geom_boxplot(aes(x = 대계열, y = 취업률_계)) +
  ## X값에 대한 Y 값은 평균(mean), 미적요소는 point, color는 'red'로 설정한 stat_summary 레이어를 추가
...
p_point + 
  ...
  ## X축은 대계열, Y축은 취업률_계로 매핑하고 평균(mean), 미적요소는 point, color는 'red'로 설정한 stat_summary 레이어를 추가
  stat_summary(aes(x = 대계열, y = 취업률_계), fun = 'mean', geom = 'point', color = 'red', size = 3) +
  ## X축은 대계열, Y축은 취업률_계로 매핑하고 중간값(median), 미적요소는 point, color는 'blue'로 설정한 stat_summary 레이어를 추가
...
legend_line <- df_입학자_long |> 
  ...
  ## 연속형 y축 스케일을 설정하는데 라벨을 scales 패키지의 comma 포맷 적용
  scale_y_continuous(labels = scales::comma) + 
  ## theme()을 사용하여 axis.text.x를 element_text()를 사용해 세부 특성 설정
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
...
df_입학자 |> filter(지역 == '전체') |>
  ...
  scale_color_manual(values = c('전문대 입학생' = 'red', '일반대 입학생' = 'blue', '석사 입학생' = 'green', '박사 입학생' = 'black'))
...
```{r fig.cap='scale을 사용한 범례 아이템 설정'}
## legend_line 객체에서
legend_line +
...
```{r fig.cap='축 라벨(label, text) 특성 설정'}
legend_line +
  ## axis.text.x를 설정하는데 element_text()을 사용하여 세부 특성 설정
  theme(axis.text.x = element_text(color = 'red', size = 10, face = 'bold'), 
  ## axis.text.y를 설정하는데 element_text()을 사용하여 세부 특성 설정
...
```{r fig.cap='축 선(line) 특성 설정'}
legend_line +
  ## axis.line.x를 설정하는데 element_line()을 사용하여 세부 특성 설정
  theme(axis.line.x = element_line(color = 'red', arrow = arrow()), 
  ## axis.line.y를 설정하는데 element_line()을 사용하여 세부 특성 설정
...
데이터를 시각화하는 `ggplot`객체에서는 기하요소를 사용하여 데이터를 각종 도형으로 표현한다. 하지만 그 시각화에서 중요하게 봐야할 부분의 강조나 데이터에 대한 세부 설명문들을 추가하면 데이터를 보는 독자들이 데이터를 이해하기가 쉬워진다. 이와 같이 데이터를 추가적으로 설명하기 위해 보조적으로 추가하는 레이어를 주석 레이어(annotate)라고 한다. 이 주석 레이어는 `annotate()`를 사용하여 생성할 수 있다.

```{r eval=FALSE}
annotate(geom, x = NULL, y = NULL, xmin = NULL, xmax = NULL, ymin = NULL, ymax = NULL, xend = NULL, yend = NULL, na.rm = FALSE, ...)
  - geom : 주석으로 사용할 기하요소 설정
  - x, y, xmin, xman, ymin, ymax, xend, yend : 기하요소 설정을 위한 위치 설정
  - na.rm : 결측치를 제거할지 여부를 결정하는 논리 값

```

위의 `annotate()`에서 사용할 수 있는 기하요소인 `geom`은 다음과 같고 각각의 기하요소에 따라 설정가능한 위치 설정 매개변수가 달라진다.

+----------------+----------------------------------------+-------------------------------------------------------------------------------------------+
| 기하요소(geom) | 주요 매개변수                          | 미적요소                                                                                  |
+================+========================================+===========================================================================================+
...
앞 절의 무변수 기하요소를 사용하여 작성한 설명을 `annotate()`을 사용하여 코딩하면 다음과 같다.
...
df_입학자_long |> filter(지역 == '전체') |>
  ...
  annotate('segment', x = '2003', y = 300000, xend = '2006', yend = 300000, 
           color = 'yellow', arrow = arrow(length = unit(0.25,'cm'), type = 'closed'), arrow.fill = 'yellow') + 
