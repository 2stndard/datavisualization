...
df_취업통계_2000 |>
  ## X축은 졸업자수, Y축은 취업자수, name은 대계열로 매핑한 plotly 객체 생성
  plot_ly(x = ~졸업자수, y = ~취업자수, name = ~대계열, color = ~대계열, colors = 'viridis') |>
  layout(title = '졸업자 대비 취업자수', margin = margins)
...
covid19_df_100 |>
  ## X축을 date, Y축을 new_cases, 범례 이름을 앞서 생성한 문자열 벡터로 매핑 
  plot_ly(x = ~date, y = ~new_cases, name = ~legend_items, color = ~legend_items, colors = 'viridis') |>
  layout(title = '최근 100일간 코로나19 확진자수', 
         ## X축 이름을 제거
         xaxis = list(title = list(text = '')),
         ## Y축 이름을 설정
...
df_취업통계_2000 |> 
  plot_ly(x = ~졸업자수, y = ~취업자수, name = ~대계열, 
          hoverinfo = 'x+y+text', text = ~대계열, color = ~대계열, colors = 'viridis', 
          ## hovertamplate의 설정
          hovertemplate = ' 졸업자:%{x}, 취업자:%{y}, 대계열:%{text}') |>
  layout(title = '졸업자 대비 취업자수', margin = margins)
```
...
p1 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, color = ~대계열, colors = 'viridis') |>
  layout(title = '졸업자 대비 취업자수')
...
subplot(
  p1 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "colors = 'viridis'", showarrow = F, xref='paper', yref='paper', xanchor = 'center')),
...
p2 <- df_취업통계_symbol |> plot_ly(x = ~졸업자수, y = ~취업자수, symbol = ~factor(symbol), color = ~factor(symbol), colors = 'viridis') |>
  layout(title = '졸업자 대비 취업자수')
...
df_취업통계_symbol |> 
  plot_ly(x = ~졸업자수, y = ~취업자수, 
          ...
          ## symbols를 심볼 이름으로 설정
          symbols = c('circle', 'circle-open', 'circle-dot', "circle-open-dot", 
                      "square", "square-open", "square-dot"), color = ~factor(symbol), colors = 'viridis') |>
  layout(title = '졸업자 대비 취업자수', margin = margins)
```
...
```{r echo = FALSE}
p1 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, stroke = 'darkblue', color = I('white')) |>
  layout(title = '졸업자 대비 취업자수')

p2 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, stroke = I('darkblue'), color = I('white')) |>
  layout(title = '졸업자 대비 취업자수')
...
covid19_df_100 |>
  plot_ly(type = 'scatter', mode = 'lines', x = ~date, y = ~new_cases,
          ## linetype을 location에 매핑
          linetype = ~location, color = ~location, colors = 'viridis') |>
  layout(title = '최근 100일간 코로나19 확진자수', 
         xaxis = list(title = list(text = NULL)), 
         yaxis = list(title = list(text = '확진자수')), 
         margin = margins)
...
p2 <- p_marker |>
  add_trace(type = 'scatter', mode = 'markers', x = ~졸업자수, y = ~취업자수, color = ~대계열, colors = 'viridis') |>
  layout(title = '졸업자 대비 취업자수')
...
p1 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, color = ~취업률, colors = 'viridis')

color_vector <- c('red', 'blue', 'green', 'yellow', 'purple', 'black', 'pink')

p2 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, color = ~취업률, colors = color_vector)

p3 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, color = ~취업률, colors = colorRamp(c('red', 'yellow', 'blue')))
...
covid19_df_100_wide |> plot_ly(x = ~date) |>
  ...
  add_trace(type = 'scatter', mode = 'lines', 
            ## Y축을 백신접종완료자_한국으로 매핑, name을 한국으로 설정
            ## X축은 초기 생성된 plotly 객체에 이미 매핑됨
            y = ~백신접종완료자_한국, name = '한국', color = '한국', colors = 'viridis') |> 
  ## type을 scatter, mode를 lines로 설정한 trace 추가
  add_trace(type = 'scatter', mode = 'lines', 
            ## Y축을 백신접종완료자_아시아로 매핑, name을 아시아로 설정
            y = ~백신접종완료자_아시아, name = '아시아', color = '아시아', colors = 'viridis') |> 
  add_trace(type = 'scatter', mode = 'lines', 
            ## Y축을 백신접종완료자_유럽로 매핑, name을 유럽로 설정
            y = ~백신접종완료자_유럽, name = '유럽', color = '유럽', colors = 'viridis') |>
  ## 제목, X, Y 축 제목, 여백 설정
  layout(title = '한국, 유럽, 아시아의 접종완료자수(인구 100명 당)', 
         xaxis = list(title = list(text = NULL)), 
         yaxis = list(title = list(text = '접종완료자수')), 
...
p_box_add <- p_box |> 
  ...
  add_trace(type = 'box', data = df_취업통계_2000 |> filter(과정구분 == '대학과정'),
            ## X, Y축을 변수에 매핑, 범례이름을 대학으로 설정
            x = ~대계열, y = ~취업률, name = '대학', color = ~factor(과정구분))

p_box_add <- p_box_add |> 
  ## df_취업통계_2000에서 대학과정 데이터를 필터링한 데이터를 사용한 박스 trace 추가
  add_trace(type = 'box', data = df_취업통계_2000 |> filter(과정구분 == '전문대학과정'), 
            ## X, Y축을 변수에 매핑, 범례이름을 대학으로 설정
            x = ~대계열, y = ~취업률, name = '전문대학', color = ~factor(과정구분))
...
p_box_quartilemethod <- p_box_quartilemethod |> 
  ## quartilemethod가 linear인 박스 trace 추가
  add_trace(type = 'box', x = ~대계열, y = ~취업률, color = ~factor('linear'), colors = 'viridis',
            quartilemethod = 'linear', name = 'linear')

p_box_quartilemethod <- p_box_quartilemethod |> 
  ## quartilemethod가 inclusive인 박스 trace 추가
  add_trace(type = 'box', x = ~대계열, y = ~취업률, color = ~factor('inclusive'),  colors = 'viridis',
            quartilemethod = 'inclusive', name = 'inclusive')

p_box_quartilemethod <- p_box_quartilemethod |> 
  ## quartilemethod가 exclusive인 박스 trace 추가
  add_trace(type = 'box', x = ~대계열, y = ~취업률, color = ~factor('exclusive'), colors = 'viridis',
            quartilemethod = 'exclusive', name = 'exclusive')
...
p_violin |> 
  ## 대학과정만 필터링한 데이터를 사용한 바이올린 trace 추가
  add_trace(data = df_취업통계_2000 |> filter(과정구분 == '대학과정'), 
            type = 'violin', x = ~대계열, y = ~취업률, name = '대학', color = '대학', colors='viridis') |> 
  ## 전문대학과정만 필터링한 데이터를 사용한 바이올린 trace 추가
  add_trace(data = df_취업통계_2000 |> filter(과정구분 == '전문대학과정'), 
            type = 'violin', x = ~대계열, y = ~취업률, name = '전문대학', color = '전문대학', colors='viridis') |> 
  ## violinmode를 group으로 설정
  layout(violinmode = 'group', 
         title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)
...
p_pie |>
  ## value와 labels를 매핑한 파이 trace 추가
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, marker = list(colors = viridis(7, option = 'D'))) |> 
  layout(title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)
...
p_pie |>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, marker = list(colors = viridis(7, option = 'D')), textinfo = 'value',
            title = list(text = "textinfo = 'value'", position = 'top center', 
                         font = list(size = 15)), 
            domain = list(x = c(0, 0.49), y = c(0.51, 1)), 
            insidetextfont = list(size = 7)) |>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, marker = list(colors = viridis(7, option = 'D')), textinfo = 'percent', 
             title = list(text = " textinfo = 'percent'", position = 'top center',
                          font = list(size = 15)), 
             domain = list(x = c(0.51, 1), y = c(0.51, 1)), 
             insidetextfont = list(size = 7))|>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, marker = list(colors = viridis(7, option = 'D')), textinfo = 'label+percent', 
            title = list(text = "textinfo = 'label+percent'", position = 'top center', font = list(size = 15)), 
            domain = list(x = c(0, 0.49), y = c(0, 0.49)), 
            insidetextfont = list(size = 7))|>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, marker = list(colors = viridis(7, option = 'D')), textinfo = 'label+value', 
...
p_pie |>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, 
            textinfo = 'label+percent', 
            ## marker 세부 속성 설정
            marker = list(colors = viridis(7, option = 'D'), 
                          line = list(color = 'white', 
                                      width = 2)
                          )
            ) |> 
...
p_pie |>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, marker = list(colors = viridis(7, option = 'D')), textinfo = 'label+percent', insidetextorientation = 'horizontal', title = list(text = "insidetextorientation = 'horizontal'", position = 'top center', font = list(size = 15)), domain = list(x = c(0, 0.49), y = c(0.51, 1)), insidetextfont = list(size = 6)) |>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, marker = list(colors = viridis(7, option = 'D')), textinfo = 'label+percent', insidetextorientation = 'radial', title = list(text = "insidetextorientation = 'radial'", position = 'top center', font = list(size = 15)), domain = list(x = c(0.51, 1), y = c(0.51, 1)), insidetextfont = list(size = 6)) |>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, marker = list(colors = viridis(7, option = 'D')), textinfo = 'label+percent', insidetextorientation = 'tangential', title = list(text = "insidetextorientation = 'tangential'", position = 'top center', font = list(size = 15)), domain = list(x = c(0, 0.49), y = c(0, 0.49)), insidetextfont = list(size = 6)) |>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, marker = list(colors = viridis(7, option = 'D')), textinfo = 'label+percent', insidetextorientation = 'auto', title = list(text = "insidetextorientation = 'auto'", position = 'top center', font = list(size = 15)), domain = list(x = c(0.51, 1), y = c(0, 0.49)), insidetextfont = list(size = 6)) |>
  layout(showlegend = FALSE) |> 
  layout(title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)
