...
코로나바이러스 데이터 셋은 R CRAN에서 공식적으로 제공하는 `coronavirus` 패키지를 통해 제공받을 수 있다.[^1]
...
# 
# 
# coronavirus_df <- read_csv("C:/R/git/datavisualization/chap3/coronavirus.csv", 
...
# covid_stat_by_nation <- coronavirus_df %>%
#   filter(type != "recovery", !is.na(code3)) %>%
#   group_by(type, continent_name, country, uid) %>%
#   summarise(cases = sum(cases), population = max(population), .groups = "drop") %>%
#   mutate(type = factor(type, levels = c("confirmed", "death"))) %>%
#   pivot_wider(names_from = type, values_from = cases) %>%
#   mutate(death_rate = death / confirmed, confirmed_rate = confirmed / population) %>%
#   filter(!is.na(country))

# covid19_vaccine_df <- read_csv(file = "C:/R/git/datavisualization/chap3/covid19_vaccine.csv",
#                             col_types = cols(Date = col_date(format = "%Y-%m-%d"),
#                                              Report_Date_String = col_date(format = "%Y-%m-%d"),
#                                              UID = col_number(),
#                                              Province_State =col_character()
#                                              )
#                             )
# covid_joined <- inner_join(covid_stat_by_nation, vaccine_stat_by_nation, by = c('country' = 'country_region', 'uid' = 'uid'))
...
# covid19_df <- covid19_df |> select(iso_code, continent, location, date, population, gdp_per_capita, total_cases, new_cases, total_deaths, new_deaths, icu_patients_per_million, hosp_patients_per_million, reproduction_rate, total_tests, new_tests, total_vaccinations, people_fully_vaccinated, total_boosters,  people_fully_vaccinated_per_hundred, total_boosters_per_hundred)
# 
# 
covid19_stat <- covid19_df |> group_by(iso_code, continent, location) |>
  summarise(population = max(population, na.rm = T), gdp_per_capita = max(gdp_per_capita, na.rm = T),
            total_cases = max(total_cases, na.rm = T), new_cases = sum(new_cases, na.rm = T),
            total_deaths = max(total_deaths, na.rm = T), new_deaths = sum(new_deaths, na.rm = T), death_per_million = round(total_deaths / population *100000, 5),
            icu_patients_per_million = last(icu_patients_per_million),
            hosp_patients_per_million = last(hosp_patients_per_million),
            reproduction_rate = last(reproduction_rate),
            total_tests = max(total_tests, na.rm = T), new_tests = sum(new_tests, na.rm = T),
            total_vaccinations = max(total_vaccinations, na.rm = T),
            people_fully_vaccinated = max(people_fully_vaccinated, na.rm = T),
            total_boosters = max(total_boosters, na.rm = T),
            fully_vaccinated_rate = people_fully_vaccinated / population,
...
[^1] : `coronavirus` 패키지에서 제공하는 데이터 셋의 자세한 내용은 해당 패키지의 Github에서 확인할 수 있다.(https://ramikrispin.github.io/coronavirus/articles/intro_coronavirus_dataset.html) 
...
```{r}
df_취업통계_2000 |>
  plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, name = ~대계열, hovertext = ~학과명, hoverinfo = 'x+y+text')
...
```{r echo = FALSE}
p1 <- df_취업통계_2000 |> plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, color = ~대계열, colors = 'Accent')

color_vector <- c('red', 'blue', 'green', 'yellow', 'purple', 'black', 'pink')

p2 <- df_취업통계_2000 |> plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, color = ~대계열, colors = color_vector)

p3 <- df_취업통계_2000 |> plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, color = ~대계열, colors = colorRamp(c('red', 'white', 'blue')))
...
```{r echo = FALSE}
p1 <- df_취업통계_2000 |> plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, color = ~취업률_계, colors = 'Accent')

color_vector <- c('red', 'blue', 'green', 'yellow', 'purple', 'black', 'pink')

p2 <- df_취업통계_2000 |> plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, color = ~취업률_계, colors = color_vector)

p3 <- df_취업통계_2000 |> plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, color = ~취업률_계, colors = colorRamp(c('red', 'white', 'blue')))
...
p1 <- df_취업통계_symbol |> plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, symbol = ~symbol)

p2 <- df_취업통계_symbol |> plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, symbol = ~factor(symbol))

subplot(
  p1 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "~symbol", showarrow = F, xref='paper', yref='paper', xanchor = 'center')),
  p2 |> colorbar(title = 'color_vector')|> layout(annotations = list(x = 0.5, y = 1.05, text = "~factor(symbol)", showarrow = F, xref='paper', yref='paper', xanchor = 'center'))
) |> hide_legend()
```
...
```{r echo = FALSE}
p1 <- df_취업통계_symbol |> plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, symbol = 'circle-open', color = I('#1f77b4'))

p2 <- df_취업통계_symbol |> plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, symbol = I('circle-open'), color = I('#1f77b4'))
...
```{r echo = FALSE}
p1 <- df_취업통계_2000 |> plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, stroke = 'black', color = I('white'))

p2 <- df_취업통계_2000 |> plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, stroke = I('black'), color = I('white'))
...
```{r echo = FALSE}
p1 <- df_취업통계_symbol |> plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, size = ~symbol, sizemode = 'diameter', alpha = 0.3, color = I('#1f77b4'))

p2 <- df_취업통계_symbol |> plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, size = ~symbol, sizemode = 'area', alpha = 0.3, color = I('#1f77b4'))
...
```{r}
covid19_df_100 |>
  plot_ly(type = 'scatter', mode = 'lines', x = ~date, y = ~new_cases, linetype = ~iso_code)
...
```{r echo = FALSE}
p1 <- df_취업통계_2000 |> plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, alpha = 0.3, color = I('#1f77b4'))

p2 <- df_취업통계_2000 |> plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, opacity = 0.3, color = I('#1f77b4'))
...
p1 <- p_line_wide |> add_trace(type = 'scatter', mode = 'markers', x = ~date, y = ~South_Korea, color = I('#1f77b4'))

p2 <- p_line_wide  |> add_trace(type = 'scatter', mode = 'markers+lines', x = ~date, y = ~South_Korea, color = I('#1f77b4'))

p3 <- p_line_wide  |> add_trace(type = 'scatter', mode = 'markers+text', x = ~date, y = ~South_Korea, text = ~South_Korea, color = I('#1f77b4'), textfont = list(size = 7))

p4 <- p_line_wide  |> add_trace(type = 'scatter', mode = 'markers+lines+text', x = ~date, y = ~South_Korea, text = ~South_Korea, color = I('#1f77b4'), textfont = list(size = 7))
...
```{r echo = FALSE}
p1 <- p_line_wide  |> add_trace(type = 'scatter', mode = 'markers+lines', x = ~date, y = ~South_Korea, color = I('#1f77b4'))

p2 <- p_line_wide  |> add_trace(type = 'scatter', mode = 'markers + lines', x = ~date, y = ~South_Korea, color = I('#1f77b4'))

subplot(
  p1 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "mode = 'lines+markers'", showarrow = F, xref='paper', yref='paper', xanchor = 'center')),
  p2 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "mode = 'lines + markers", showarrow = F, xref='paper', yref='paper', xanchor = 'center'))
) |> layout(showlegend = FALSE)
```
...
p_marker |>
  add_trace(type = 'scatter', mode = 'markers', x = ~졸업자_계, y = ~취업자_합계_계,
...
p_line |> 
  add_trace(type = 'scatter', mode = 'lines', x = ~date, y = ~new_cases, color = ~ iso_code)
```
...
```{r}
fig <- add_trace(p_line_wide, type = 'scatter', mode = 'lines', x = ~date, y = ~South_Korea, name = '한국')

fig <- fig |> add_trace(type = 'scatter', mode = 'lines', x = ~date, y = ~Asia, name = '아시아')

fig <- fig |> add_trace(type = 'scatter', mode = 'lines', x = ~date, y = ~Europe, name = '유럽')
...
subplot(
  ...
  p2 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "text = ~한국", showarrow = F, xref='paper', yref='paper', xanchor = 'center'))
) |> hide_legend()
...
```{r}
p_bar <- covid19_df_100 |> mutate(yearmonth = tsibble::yearmonth(date)) |> group_by(iso_code, yearmonth) |>
...
```{r}
p_bar_mode <- covid19_df_100 |> mutate(yearmonth = lubridate::floor_date(date, "month")) |> group_by(iso_code, yearmonth) |>
  summarise(new_cases = sum(new_cases)) |> plot_ly()
...
# plot_ly() |> add_trace(type = 'bar', x = 2000:2021, y = diff(df_입학자[df_입학자$지역 == '전체', ]$일반대학), 
#                        color = I(ifelse(diff(df_입학자[df_입학자$지역 == '전체', ]$일반대학)> 0, "red","blue"))) |> 
...
```{r eval = FALSE}
fig <- df_취업통계_2000 |> group_by(과정구분, 대계열) |>
  summarise(졸업자합계 = sum(졸업자_계)) |> 
  pivot_wider(names_from = 과정구분, values_from = 졸업자합계) |> 
  plot_ly()

fig <- fig |> add_trace(type = 'bar', x = ~대계열, y = ~전문대학과정, 
                        name = '전문대학과정', color = I('gray40'))

fig <- fig |> add_trace(type = 'bar', x = ~대계열, y = ~대학과정, 
                        name = '대학과정', color = I('darkred'))

fig <- fig |> add_trace(type = 'bar', x = ~대계열, y = ~대학원과정, 
                        name = '대학원과정', color = I('gray60'))
...
fig <- fig |> add_trace(type = 'bar', x = ~대계열, y = ~전문대학과정, 
                        name = '전문대학과정', color = I('gray30'), 
                        text = ~전문대학과정, textposition = 'inside', 
                        textfont = list(color = 'white'))

fig <- fig |> add_trace(type = 'bar', x = ~대계열, y = ~대학과정, 
                        name = '대학과정', color = I('darkred'), 
                        text = ~대학과정, textposition = 'inside', 
                        textfont = list(color = 'white'))

fig <- fig |> add_trace(type = 'bar', x = ~대계열, y = ~대학원과정, 
                        name = '대학원과정', color = I('gray70'), 
                        text = ~대학원과정, textposition = 'inside', 
                        textfont = list(color = 'white'))
...
```{r}
df_취업통계_2000 |> group_by(과정구분, 대계열) |>
  summarise(졸업자합계 = sum(졸업자_계)) |> 
  pivot_wider(names_from = 과정구분, values_from = 졸업자합계) |> 
  plot_ly() |>
  add_trace(type = 'bar', x = ~대계열, y = ~대학과정, name = '대학과정', 
            text = ~대학과정, textposition = 'outside', 
            textfont = list(color = 'black', size = 15), texttemplate = '%{text:,}') 

```

#### marker

앞서 스캐터 trace에서 산점도, 선 그래프를 생성할 때 `marker`를 사용하여 점과 선의 형태를 설정하였다. 그러나 막대 trace에서의 `makrker`는 막대 자체를 의미한다. 따라서 `marker`로 설정 가능한 type은 막대의 내부 색, 외부 외곽선 등의 세부 설정을 위한 type을 지정할 수 있다.
...
subplot(
  ...
  p2 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "boxmean = 'sd'", showarrow = F, xref='paper', yref='paper', xanchor = 'center'))
) |> hide_legend()
...
```{r}
p_box |> add_trace(type = 'box', x = ~대계열, y = ~취업률_계, 
                   notched = TRUE, notchewidth = 0.2, notchespan = 0.5, 
                   whiskerwidth = 0.3)
...
`line`은 막대 trace에서 외곽선을 설정하는 type이다. 이 type에서는 세부 선의 색과 두께를 설정할 수 있다.
...
바이올린 trace에서도 박스 trace와 같이 데이터를 점으로 표현할 수 있는데 이를 제어하기 위한 type이 `marker`와 `jitter`이고 바이올린의 외곽선을 설정하는 type이 `line`이다. 이들의 설정은 박스 trace와 동일하나 `boxpoints`가 `points`로 바뀐다는 점이 다르다.
...
앞선 여타 trace와는 달리 파이 trace에는 X, Y축에 매핑되는 `x`, `y` type의 설정이 없다. 대신 필수적 type으로 `values`와 `labels`를 설정해야하는데 `values`에 매핑되거나 설정된 수치 벡터에 따라 원을 각각의 슬라이스로 구분하게 되고 `labels`에 매핑하거자 설정한 벡터가 각각의 trace 이름으로 설정된다. 또 원 그래프에 표시되는 데이터 값은 기본적으로 백분률 값이 표시된다.

```{r}
p_pie <- df_입학자_long |> filter(연도 == '2021', 지역 == '전체', 학교종류 %in% c('전문대학', '일반대학', '석사', '박사')) |> plot_ly()

p_pie |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류)

```

#### textinfo

앞의 원 그래프에서 원 안에 표현된 데이터는 `values`에 매핑된 데이터의 백분율이 표시되었다. 이 데이터의 표시를 설정하는 type이 `textinfo`이다. `textinfo`에서 설정사 가능한 값은 'label', 'text', 'value', 'percent'의 네 가지이고 이 네가지를 `+`를 사용하여 동시에 사용할 수 있다. 'label'은 `labels`로 설정된 값, `text`는 `text`로 설정된 값, 'value'는 `values`에 설정된 값, 'percent'는 백분률 값이 표시된다.

```{r eval=FALSE}
p_pie |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'value')

p_pie |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'percent')

p_pie |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'label+percent')

p_pie |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'label+value')

```

```{r echo=FALSE}
p_pie |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'value', title = list(text = "textinfo = 'value'", position = 'top center', font = list(size = 15)), domain = list(x = c(0, 0.4), y = c(0.6, 1)), insidetextfont = list(size = 10)) |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'percent', title = list(text = " textinfo = 'percent'", position = 'top center', font = list(size = 15)), domain = list(x = c(0.6, 1), y = c(0.6, 1)), insidetextfont = list(size = 10))|>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'label+percent', title = list(text = "textinfo = 'label+percent'", position = 'top center', font = list(size = 15)), domain = list(x = c(0, 0.4), y = c(0, 0.4)), insidetextfont = list(size = 10))|>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'label+value', title = list(text = "textinfo = 'label+value'", position = 'top center', font = list(size = 15)), domain = list(x = c(0.6, 1), y = c(0, 0.4)), insidetextfont = list(size = 10)) |>
  layout(showlegend = FALSE)
...
`hole`은 원 그래프의 내부에 표시되는 구멍의 크기를 설정하는 type이다. `hole`이 설정되지 않거나 0으로 설정되면 완전한 원 그래프로 표시되지만 0부터 1사이의 값이 설정되면 그 크기만큼 빈 원이 생성되면서 도넛 그래프로 전환된다.

```{r eval=FALSE}
p_pie |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'label+percent', hole = 0.4)
```
...
```{r eval=FALSE}
p_pie |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'label+percent', marker = list(colors = c('red', 'green', 'blue', 'black'), line = list(color = 'white', width = 1)))
```
...
```{r eval=FALSE}
p_pie |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'label+percent', insidetextorientation = 'horizontal')

p_pie |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'label+percent', insidetextorientation = 'radial')

p_pie |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'label+percent', insidetextorientation = 'tangential')

p_pie |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'label+percent', insidetextorientation = 'auto')

```

```{r echo=FALSE}
p_pie |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'label+percent', insidetextorientation = 'horizontal', title = list(text = "insidetextorientation = 'horizontal'", position = 'top center', font = list(size = 15)), domain = list(x = c(0, 0.4), y = c(0.6, 1)), insidetextfont = list(size = 8)) |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'label+percent', insidetextorientation = 'radial', title = list(text = "insidetextorientation = 'radial'", position = 'top center', font = list(size = 15)), domain = list(x = c(0.6, 1), y = c(0.6, 1)), insidetextfont = list(size = 8)) |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'label+percent', insidetextorientation = 'tangential', title = list(text = "insidetextorientation = 'tangential'", position = 'top center', font = list(size = 15)), domain = list(x = c(0, 0.4), y = c(0, 0.4)), insidetextfont = list(size = 8)) |>
  add_trace(type = 'pie', values = ~입학생수, labels = ~학교종류, textinfo = 'label+percent', insidetextorientation = 'auto', title = list(text = "insidetextorientation = 'auto'", position = 'top center', font = list(size = 15)), domain = list(x = c(0.6, 1), y = c(0, 0.4)), insidetextfont = list(size = 8))
...
인디케이터 trace를 사용하기 위해서는 `add_trace()`의 `type`을 'indicator'로 설정함으로써 사용할 수 있다.

#### mode, value

인디케이터 trace에서 제공하는 표시 방법은 수치 표시, 증감량 표시, 게이지 표시의 세가지 방법을 제공한다. 이들을 설정하는 type이 `mode`이다. 이 `mode`는 수치를 표현하는 'number', 증감을 표현하는 'delta', 게이지를 사용하는 'gauge'의 세 가지를 설정할 수 있고 각각은 `+`를 사용하여 조합할 수 있다.
...
```{r}
plot_ly() |>
  add_trace(type = 'indicator', mode = "gauge+number+delta", gauge = list(shape = 'angular'), value = 200, delta = list(reference = 250))
```
...
```{r}
p_title_1 |> layout(title = list(text = "<span style = 'font-size:15pt'><span style = 'color:red;font-weight:bold;'> 졸업수</span><span style = 'font-size:10pt'> 대비</span> <span style = 'color:blue;font-weight:bold;'>취업률</span></span>", 
                                 x = 0.1, xanchor = 'left', yanchor = 'top')
                    )
```
...
p_title_2 <- p_title_2 |> add_trace(type = 'scatter', mode = 'lines', 
                                    x = ~연도, y = ~석사, name = '석사', showlegend = FALSE)
...
p_title_2 <- add_trace(p_title_line, type = 'scatter', mode = 'lines', 
                       x = ~연도, y = ~전문대학, name = '전문대학', legendgroup = '학부')
...
`plotly`에서 사용하는 색의 설정은 색 이름 설정, 16진수로 설정된 RGB값 설정, `rgb()` 함수를 사용한 RGB값의 설정 세 가지 방법이 주로 사용된다. 이외에도 색조, 채도, 명도(lightness)를 사용하는 `hsl()`을 사용하는 방법, 색조, 채도, 명도(value)를 사용하는 `hsv()`를 사용하는 방법도 있다. `plotly`에서 사용이 가능한 색 이름은 W3.org에서 제공하는 CSS 색 이름을 사용한다.[^6]
...
p_title_1 |> layout(title = list(text = '<b>졸업자 대비 취업자수</b>', x = 0.1, 
                    ...
                    paper_bgcolor = 'floralwhite', plot_bgcolor = 'antiquewhite', 
                    xaxis = list(title = list(text = '졸업자', font = list(family = '나눔명조', size = 20, color = 'Crimson')
                                              ),  ## 정상적 방법
                                 zerolinewidth = 3, zerolinecolor = 'white'), 
                    yaxis = list(title = '취업자', gridcolor = 'white', gridwidth = 2) ## 약식 방법
                    )
```
...
+------------------------+-----------------------------------------------------------------+------------------------------------------------------------------------------------+---------------------+
| x, y                   | 주석을 설정할 X축과 Y축의 좌표를 설정                           |                                                                                    |                     |
+------------------------+-----------------------------------------------------------------+------------------------------------------------------------------------------------+---------------------+
| ax, ay                 | 주석이 실제로 위치할 좌표의 상대적 위치                         |                                                                                    |                     |
+------------------------+-----------------------------------------------------------------+------------------------------------------------------------------------------------+---------------------+
| xanchor                | 주석 문자열의 수평 위치 맟춤값 설정                             | 'auto'는 자동설정, 'left'는 왼쪽 맞춤, 'center'는 중간 맞춤, 'right'는 오른쪽 맞춤 |                     |
+------------------------+-----------------------------------------------------------------+------------------------------------------------------------------------------------+---------------------+
| yanchor                | 주석 문자열의 수직 위치 맟춤값 설정                             | 'auto'는 자동설정, 'top'은 상단 맞춤, 'middle'은 중간 맞춤, 'bottom'은 하단 맞춤   |                     |
+------------------------+-----------------------------------------------------------------+------------------------------------------------------------------------------------+---------------------+
| xref, yref             | `x`, `y`, `ax`, `ay`의 절대 좌표와 상대 좌표의 참조 기준을 설정 | 'x', 'y'는 축 스케일을 참조, 'paper'는 플로팅 영역을 0부터 1까지 정규화하여 참조   |                     |
+------------------------+-----------------------------------------------------------------+------------------------------------------------------------------------------------+---------------------+
| arrowcolor             | 화살촉의 색 설정                                                |                                                                                    |                     |
+------------------------+-----------------------------------------------------------------+------------------------------------------------------------------------------------+---------------------+
| arrowhead              | 화살촉의 모양 설정                                              | 0부터 8까지의 수치                                                                 |                     |
+------------------------+-----------------------------------------------------------------+------------------------------------------------------------------------------------+---------------------+
| arrowside              | 화살촉 위치 설정                                                | 'end', 'start', 'none'을 '+'로 조합 가능                                           |                     |
|                        |                                                                 |                                                                                    |                     |
|                        |                                                                 | 'end'는 화살표 직선 끝에 화살촉, 'head'는 화살표 직선 앞에 화살촉을 설정           |                     |
+------------------------+-----------------------------------------------------------------+------------------------------------------------------------------------------------+---------------------+
...
```{r echo=FALSE}
p1 <- plot_ly() %>% 
  add_markers(x = 1,  y = 1,  showlegend = F) %>% 
  add_markers(x = 1,  y = 2,  showlegend = F) %>% 
  add_markers(x = 1,  y = 3,  showlegend = F) %>% 
...
```{r}
df_annotation <- df_취업통계 |> filter(졸업자_계 < 500, 학과명 == '경영학과') |> select(과정구분, 학과명, 졸업자_계, 취업자_합계_계)

p_title_1 |> 
  layout(title = list(text = '<b>졸업자 대비 취업자수</b>', x = 0.1, 
                      xanchor = 'left', yanchor = 'top', size = 15), 
         font = list(family = "나눔고딕", color = 'black', size = 10), 
         annotations = list(text = ~paste0(df_annotation$과정구분, ':', df_annotation$학과명), x = df_annotation$졸업자_계, y = df_annotation$취업자_합계_계, ax = 30, ay = -30)
         )
```
...
plot_ly() %>% 
  ...
  add_annotations(x= 0.5, y= 1, xref = "paper", yref = "paper",
    text = "<b>xref = \"paper\", yref = \"paper\" 일 때 (0.5, 1)의 위치</b>", showarrow = F) %>% 
  add_annotations(x= 0.5, y= 1, xref = "x", yref = "y",
    text = "xref = \"x\", yref = \"y\" 일 때 (0.5, 1)의 위치", showarrow = T, 
    ax = 20, ay = -40) %>% 
  layout(
...
p_title_1 |> 
  layout(title = list(text = '<b>졸업자 대비 취업자수</b>', x = 0.1, 
         ...
         font = list(family = "나눔고딕", color = 'black', size = 10), 
         annotations = list(text = ~paste0('<b>', df_annotation$과정구분, ':', df_annotation$학과명, '</b>'), 
                            x = df_annotation$졸업자_계, y = df_annotation$취업자_합계_계, ax = 30, ay = -30), 
         margin = list(t = 50)) |>
...
p_title_1 |> 
  layout(title = list(text = '<b>졸업자 대비 취업자수</b>', x = 0.1, 
                      xanchor = 'left', yanchor = 'top', size = 15), 
         font = list(family = "나눔고딕", color = 'black', size = 10), 
         annotations = list(text = ~paste0('<b>', df_annotation$과정구분[1], ':', df_annotation$학과명[1], '</b>'), x = df_annotation$졸업자_계[1], y = df_annotation$취업자_합계_계[1], ax = 30, ay = -30, 
...
| type           | 설명                                             | type 값 및 설명                                                     | 세부 type |
|----------------|--------------------------------------------------|---------------------------------------------------------------------|-----------|
| type           | 그려질 도형의 형태 설정                          | 'circle'은 원, 'rect'는 사각형, 'path'는 SVG path, 'line'은 선 설정 |           |
| name           | 그려질 도형의 이름 설정                          |                                                                     |           |
| x0, x1, y0, y1 | 도형의 X, Y축 시작점(x0, y0)과 끝점(x1, y1) 설정 |                                                                     |           |
| layer          | 그려지는 도형 레이어의 위치를 설정               | 'above'는 다른 도형보다 위, 'below'는 다론 도형보다 아래에 위치     |           |
|                |                                                  |                                                                     |           |
|                |                                                  |                                                                     |           |
|                |                                                  |                                                                     |           |
