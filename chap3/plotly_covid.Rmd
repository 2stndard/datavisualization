...
p1 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, color = ~대계열, colors = 'Blues') |>
  layout(title = '졸업자 대비 취업자수')
...
p3 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, color = ~대계열, colors = colorRamp(c('black',  'lightblue'))) |>
  layout(title = '졸업자 대비 취업자수')
...
p2 <- df_취업통계_symbol |> plot_ly(x = ~졸업자수, y = ~취업자수, symbol = ~factor(symbol), color = ~factor(symbol), colors = 'Blues') |>
  layout(title = '졸업자 대비 취업자수')
...
covid19_df_100 |>
  mutate(location = fct_relevel(covid19_df_100$location, '아프리카', '오세아니아',  '남미', '북미', '유럽','아시아', '한국')) |>
  plot_ly(type = 'scatter', mode = 'lines', x = ~date, y = ~new_cases,
          ## linetype을 location에 매핑
...
p1 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, color = ~취업률, colors = 'Blues')

color_vector <- c('red', 'blue', 'green', 'yellow', 'purple', 'black', 'pink')

p2 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, color = ~취업률, colors = color_vector)

p3 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, color = ~취업률, colors = colorRamp(c('black', 'skyblue')))
...
plot_ly() |> 
  ...
  add_trace(type = 'bar', 
            ...
            color = I(ifelse(diff(diff_sam) > 0, "lightblue", "darkblue"))
                       ) |> 
...
covid19_df_100 |>
  mutate(location = fct_relevel(covid19_df_100$location, '오세아니아','아프리카',  '남미', '북미', '유럽','아시아', '한국')) |>
  plot_ly(type = 'scatter', mode = 'lines', x = ~date, y = ~new_cases,
          ## linetype을 location에 매핑
          linetype = ~location, linetypes = c(7, 6, 5, 4, 3, 2, 1),
          color = ~location, colors = 'Blues') |> 
...
## 서브 플롯들의 Y축 범위 설정
p1_blue <- p1_blue |> layout(yaxis = list(range = c(0, 2000000)))
