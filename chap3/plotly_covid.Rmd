...
covid19_df <- read_csv(file = "C:/R/git/datavisualization/chap3/owid-covid-data.csv",
                            col_types = cols(Date = col_date(format = "%Y-%m-%d")
                                             )
                            )
...
covid19_stat_df <- covid19_df |> group_by(iso_code, continent, location) |>
...
```{r}
covid19_df |> filter(iso_code == 'KOR') |> 
...
```{r}
covid19_stat_df |> 
  plot_ly(x = ~death_per_million, y = ~fully_vaccinated_rate)

# covid_joined |>
#   plot_ly(x = ~death_rate, y = ~fully_vaccinated_ratio)
# 
# 
# View(covid19_stat_df)
```
...
```{r}
covid_joined |>  
  plot_ly(x = ~death_rate, y = ~fully_vaccinated_ratio, name = ~continent_name.x)

covid19_stat_df |> 
  plot_ly(x = ~death_per_million, y = ~fully_vaccinated_rate, name = ~continent)
...
```{r}
legend_items <- covid19_stat_df |> 
  mutate(cont_kor = case_when(
...
covid19_stat_df |> 
  plot_ly(x = ~death_per_million, y = ~fully_vaccinated_rate, name = legend_items)

# covid_joined |> 
#   plot_ly(x = ~death_rate, y = ~fully_vaccinated_ratio, name = legend_items)
...
```{r}
covid19_stat_df |> 
  plot_ly(x = ~death_per_million, y = ~fully_vaccinated_rate, name = legend_items, hovertext = ~location)

# covid_joined |> 
#   plot_ly(x = ~death_rate, y = ~fully_vaccinated_ratio, name = legend_items, hovertext = ~country)
```
...
```{r}
covid19_stat_df |> 
  plot_ly(x = ~death_per_million, y = ~fully_vaccinated_rate, name = legend_items, hovertext = ~location, hoverinfo = 'x+y+name')

# df_취업통계 |> filter(졸업자_계 < 500) |>
...
```{r}
covid19_stat_df |> 
  plot_ly(x = ~death_per_million, y = ~fully_vaccinated_rate, name = legend_items, hovertext = ~location, hoverinfo = 'x+y+text')
...
```{r}
covid19_stat_df |> 
  plot_ly(x = ~death_per_million, y = ~fully_vaccinated_rate, name = legend_items, hovertext = ~location, hoverinfo = 'x+y+text', hovertemplate = ' 십만명당사망자:%{x}, 백신접종률:%{y}')
...
```{r echo = FALSE}
p1 <- df_취업통계 |> filter(졸업자_계 < 500) |> 
  plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, color = 'black')

p2 <- df_취업통계 |> filter(졸업자_계 < 500) |> 
  plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, color = I('black'))
...
```{r}
df_입학자_long |> filter(지역 == '전체', 학교종류 %in% c('전문대학', '일반대학', '석사', '박사')) |> 
  plot_ly(type = 'scatter', mode = 'lines', x = ~연도, y = ~입학생수, linetype = ~학교종류)
...
p_line |> add_trace(type = 'scatter', mode = 'markers', x = ~연도, y = ~전문대학)

p_line  |> add_trace(type = 'scatter', mode = 'markers+lines', x = ~연도, y = ~전문대학)

p_line  |> add_trace(type = 'scatter', mode = 'markers+text', x = ~연도, y = ~전문대학)

p_line  |> add_trace(type = 'scatter', mode = 'markers+lines+text', x = ~연도, y = ~전문대학)
...
```{r eval = FALSE}
p_list |> add_trace(type = 'scatter', mode = 'lines+markers', 
                    x = ~연도, y = ~전문대학, x = ~졸업자_계, y = ~취업자_합계_계)
...
fig <- fig |> add_trace(type = 'scatter', mode = 'lines', x = ~연도, y = ~일반대학, name = '대학')

fig <- fig |> add_trace(type = 'scatter', mode = 'lines', x = ~연도, y = ~석사, name = '석사')

fig <- fig |> add_trace(type = 'scatter', mode = 'lines', x = ~연도, y = ~박사, name = '박사')
...
fig <- add_trace(p_line_wide, type = 'scatter', mode = 'lines', 
                 ...
                 line = list(color = 'gray'), showlegend = FALSE)
...
fig <- fig |> add_trace(type = 'scatter', mode = 'lines', 
                        x = ~연도, y = ~석사, name = '석사', 
                        line = list(color = 'red'), showlegend = TRUE)
...
add_trace(p_line_wide, type = 'scatter', mode = 'text', 
          x = ~연도, y = ~전문대학, text = '전문대학', showlegend = FALSE)
...
```{r}
p_bar <- df_입학자_long |> filter(지역 == '전체', 학교종류 %in% c('전문대학', '일반대학', '석사', '박사'), 연도 %in% c(2001, 2006, 2011, 2016, 2021)) |> 
  plot_ly()
...
p_bar |> add_trace(type = 'bar', x = ~연도, y = ~입학생수, 
                   color = ~학교종류, opacity = 0.5, colors = 'viridis') |>
...
fig <- fig |> add_trace(type = 'bar', x = ~연도, y = ~일반대학, 
                        name = '일반대학', color = I('red')) 

fig <- fig |> add_trace(type = 'bar', x = ~연도, y = ~전문대학, 
                        name = '전문대학', color = I('gray70'))

fig <- fig |> add_trace(type = 'bar', x = ~연도, y = ~석사, 
                        name = '석사', color = I('gray80'))

fig <- fig |> add_trace(type = 'bar', x = ~연도, y = ~박사, 
                        name = '박사', color = I('gray90'))
...
fig <- fig |> add_trace(type = 'bar', x = ~연도, y = ~일반대학, 
                        name = '일반대학', color = I('red'), 
                        text = ~일반대학, textposition = 'outside', 
...
df_입학자 |> filter(지역 == '전체', 연도 %in% c(2001, 2006, 2011, 2016, 2021)) |> plot_ly() |>
  add_trace(type = 'bar', x = ~연도, y = ~일반대학, name = '일반대학', 
            text = ~일반대학, textposition = 'outside', 
...
df_입학자 |> filter(지역 == '전체', 연도 %in% c(2001, 2006, 2011, 2016, 2021)) |> plot_ly() |>
  add_trace(type = 'bar', x = ~연도, y = ~일반대학, name = '일반대학', 
            ...
            marker = list(color = 'aliceblue',
                          line = list(color = 'aquablue', width = 1.5))
            ) 
