...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE, fig.width = 6.5, dpi = 120)
library(showtext)
showtext_auto()
library(tidyverse)
library(readxl)
...
legend_items <- covid19_df_100 |> 
  mutate(legend_name = case_when(
    ...
    iso_code == 'OWID_AFR' ~ '아프리카')) |>
  select(legend_name) |>
  pull()

legend_items <- fct_relevel(legend_items, '한국', '아시아', '유럽', '북미', '남미', '오세아니아', '아프리카')
...
df_취업통계_2000 |> 
  plot_ly(x = ~졸업자수, y = ~취업자수, name = ~대계열, 
          hoverinfo = 'x+y+text', text = ~대계열, hovertemplate = ' 졸업자:%{x}, 취업자:%{y}, 대계열:%{text}') |>
  layout(title = '졸업자 대비 취업자수', margin = margins)
```
...
스캐터 trace에서 약 50여개가 넘는 속성을 사용하여 설정이 가능하다. 이 50여개의 속성 중에는 다시 세부 속성을 설정하는 속성들이 있고 총 4단계까지 내려가는 속성도 존재한다. 세부 하위 type은 `list()`를 사용하여 세부 타입을 설정할 수 있다. 이 모든 type을 다 설명할 수는 없기 때문에 주요한 type을 위주로 설명한다. 전체 type은 plotly.com에 수록되어 있다.[^5] 스캐터 trace에서 사용하는 속성 중 주요 속성은 다음과 같다.
...
p_line_wide |> add_trace(type = 'scatter', mode = 'markers', 
                         x = ~date, y = ~확진자_한국) |>
  layout(title = '최근 100일간 코로나19 확진자수', 
         yaxis = list(title = list(text = '확진자수')), 
         margin = margins)

p_line_wide  |> add_trace(type = 'scatter', mode = 'markers+lines', 
                          x = ~date, y = ~확진자_한국) |>
  layout(title = '최근 100일간 코로나19 확진자수', 
         yaxis = list(title = list(text = '확진자수')), 
         margin = margins)

p_line_wide  |> add_trace(type = 'scatter', mode = 'markers+text', 
                          x = ~date, y = ~확진자_한국, text = ~확진자_한국) |>
  layout(title = '최근 100일간 코로나19 확진자수', 
         yaxis = list(title = list(text = '확진자수')), 
         margin = margins)

p_line_wide  |> add_trace(type = 'scatter', mode = 'markers+lines+text', 
                          x = ~date, y = ~확진자_한국, text = ~확진자_한국) |>
  layout(title = '최근 100일간 코로나19 확진자수', 
         yaxis = list(title = list(text = '확진자수')), 
         margin = margins)
...
컬러바 설정에 사용되는 세부 속성은 현재(22.03) 43개가 제공된다. 이에 대한 세부 속성과 설명은 `plotly` 매뉴얼을 참조하라.[^6]

[^6]: <https://plotly.com/r/reference/#scatter-marker-colorbar>
...
선 그래프의 경우는 선을 너무 많이 그려서 꼬이기 시작하면 오히려 데이터를 확인하는데 방해가 되는 시각화가 되는 경우가 있다. 이를 스파게티 선 그래프라고 하는데 하나의 선 그래프에는 보통 5\~6개 이상의 선을 넣지 않는 것이 일반적이다. 따라서 선 그래프의 경우는 변수 매핑에 의해 단번에 선 그래프를 생성하는 방법도 많이 사용하지만 긴 형태의 데이터프레임으로 변환하여야 하고 매핑된 변수의 카테고리가 너무 많으면 스파게티 선 그래프가 생성된다. 따라서 사용자가 그리기를 원하는 3\~4개의 단일 선 그래프를 겹쳐 그리는 방법도 많이 사용된다.
...
covid19_df_100_wide |> 
  ...
  add_trace(type = 'scatter', mode = 'lines', y = ~백신접종완료자_아시아, 
            name = '아시아') |> 
  add_trace(type = 'scatter', mode = 'lines', y = ~백신접종완료자_유럽, 
            name = '유럽') |>
...
fig <- fig |> add_trace(data = covid19_df |> filter(iso_code == 'KOR'), type = 'scatter', mode = 'lines', 
...
```{r}
p_bar <- covid19_df_100 |> mutate(yearmonth = tsibble::yearmonth(date)) |> 
...
p_bar |>
  ...
  layout(title = list(text = '지역별 코로나19 확진자수'),
         xaxis = list(title = list(text = '지역'), 
                      ticktext = c('한국', '아시아', '유럽', '북미', '오세아니아', '남미', '아프리카'), 
                      tickvals = c('KOR', 'OWID_ASI', 'OWID_EUR', 'OWID_NAM', 'OWID_OCE', 'OWID_SAM', 'OWID_AFR')), 
         yaxis = list(title = '확진자수'), 
         margin = margins)
...
박스 trace도 막대 trace와 같이 그룹화한 변수를 `color`나 `linetype`등의 속성에 매핑시켜서 다수의 박스 trace를 한번에 만들거나 각각의 박스 trace를 추가하여 그릴 수 있다. 이렇게 여러개의 박스 trace를 그릴 때 그 구성 형태를 결정하기 위해서는 차후 설명할 `layout()`의 `boxmode` 속성을 설정함으로써 가능하다.
...
plot_ly() |>
  add_trace(type = 'indicator', mode = "gauge+number",
            title = list(text = '조사 만족도', font = list(size = 15), align = 'right'), 
            gauge = list(shape = 'bullet', 
                         ...
                         threshold = list(line = list(color = 'yellow', width = 3),
                                          value = 4.5)
                         ), 
            value = 3.5) |>
  ...
  add_trace(type = 'indicator', mode = "gauge+delta+number", 
            ...
            gauge = list(shape = 'angular', 
                         ...
                         threshold = list(line = list(color = 'yellow', width = 3),
                                          value = 4.5)), 
            value = 3.5, 
            delta = list(reference = 2, position = "top", 
...
+-----------------------------------+----------------------------------------------------------+------------------------------------------------------+-------------------------------------------+
| tickvals, ticktext                | 눈금자의 위치, 표시문자 설정                             | 데이터 프레이멸, 리스트, 벡터                        |                                           |
+-----------------------------------+----------------------------------------------------------+------------------------------------------------------+-------------------------------------------+
...
p_title_2 |> 
  layout(title = list(text = '<b>최근 100일간 코로나19 확진자수</b>', x = 0.1,
         ...
         legend = list(title = list(text = '지역', side = 'top')),
         paper_bgcolor = 'floralwhite', plot_bgcolor = 'antiquewhite',
...
p_title_2 |> 
  layout(title = list(text = '<b>최근 100일간 코로나19 확진자수</b>',
         ...
         yaxis = list(title = '취업자', gridcolor = 'white', gridwidth = 2, 
                      zerolinewidth = 5, zerolinecolor = 'darkred')
         )
```
...
p_title_1 |> 
  layout(title = list(text = '<b>최근 100일간 코로나19 확진자수</b>', 
         ...
         yaxis = list(title = '취업자', ticks="inside", tick0 = 100, dtick = 100)
         )
```
...
```{r}
df_annotation <- df_취업통계_2000 |> filter(학과명 == '정보통신학과') |> 
...
p_title_1 |> 
  ...
  add_annotations(xref = 'paper', yref = 'paper', 
                  text = '정보통신공학과 데이터', x = 0.05, y = 1.05,
                  showarrow = FALSE)
```
...
p_title_1 |> 
  ...
  add_annotations(text = ~paste0('<b>', df_annotation$과정구분[3], ':', df_annotation$학과명[3], '</b>'), 
                  x = df_annotation$졸업자수[3], 
                  y = df_annotation$취업자수[3], 
                  ax = 30, ay = -30, 
                  arrowhead = 5, arrowcolor = 'green', 
                  arrowsize = 0.8, 
                  font = list(family = '나눔고딕', size = 15, color = 'green')) |>
  add_annotations(text = ~paste0('<b>', df_annotation$과정구분[4], ':', df_annotation$학과명[4], '</b>'), 
                  ...
                  arrowsize = 0.8, 
                  font = list(family = '나눔명조', size = 15, color = 'black'))
```
...
```{r}
covid19_df_100 |>
  plot_ly(x = ~date, y = ~new_cases, color = ~iso_code) |> 
  ...
  layout(title = list(text = '지역별 코로나19 확진자수'),
         ...
         shapes = list(list(type = 'rect',
                            x0 = as.Date('2022-01-01'), x1 = as.Date('2022-01-31'),
                            y0 = 0, y1 = 2000000, fillcolor = "red",
                            line = list(color = "red"), opacity = 0.2), 
                       list(type = 'line', x0 = 0, x1= 1, 
                            y0 = 1250000, y1 = 1250000, 
                            xref = 'paper', yref = 'y', 
...
p1 <- p_line_wide |> add_trace(type = 'scatter', mode = 'lines', x = ~date, 
                               y = ~확진자_한국, color = I('#1f77b4')) |> 
  layout(xaxis = list(tickfont = list(size = 10), title = list(title = NULL)), 
         yaxis = list(title = list(text = '확진자수')))

p2 <- p_line_wide  |> add_trace(type = 'scatter', mode = 'lines', x = ~date, 
                                y = ~확진자_아시아, color = I('#1f77b4')) |> 
  layout(xaxis = list(tickfont = list(size = 10), title = list(title = NULL)), 
         yaxis = list(title = list(text = '확진자수')))

p3 <- p_line_wide  |> add_trace(type = 'scatter', mode = 'lines', x = ~date, 
                                y = ~확진자_유럽, color = I('#1f77b4')) |> 
  layout(xaxis = list(tickfont = list(size = 10), title = list(title = NULL)), 
         yaxis = list(title = list(text = '확진자수')))

p4 <- p_line_wide  |> add_trace(type = 'scatter', mode = 'lines', x = ~date, 
                                y = ~확진자_북미, color = I('#1f77b4')) |> 
  layout(xaxis = list(tickfont = list(size = 10), title = list(title = NULL)), 
         yaxis = list(title = list(text = '확진자수')))

p5 <- p_line_wide  |> add_trace(type = 'scatter', mode = 'lines', x = ~date, 
                                y = ~확진자_남미, color = I('#1f77b4')) |> 
  layout(xaxis = list(tickfont = list(size = 10), title = list(title = NULL)), 
         yaxis = list(title = list(text = '확진자수')))

p6 <- p_line_wide  |> add_trace(type = 'scatter', mode = 'lines', x = ~date, 
                                y = ~확진자_오세아니아, color = I('#1f77b4')) |> 
  layout(xaxis = list(tickfont = list(size = 10), title = list(title = NULL)), 
         yaxis = list(title = list(text = '확진자수')))

p7 <- p_line_wide  |> add_trace(type = 'scatter', mode = 'lines', x = ~date, 
                                y = ~확진자_아프리카, color = I('#1f77b4')) |> 
  layout(xaxis = list(tickfont = list(size = 10), title = list(title = NULL)), 
         yaxis = list(title = list(text = '확진자수')))

subplot(
  p1 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "한국'", 
                                  showarrow = F, xref='paper', yref='paper', 
                                  xanchor = 'center')),
  p2 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "아시아'", 
                                  showarrow = F, xref='paper', yref='paper', 
                                  xanchor = 'center')),
  p3 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "유럽'", 
                                  showarrow = F, xref='paper', yref='paper', 
                                  xanchor = 'center')),
  p4 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "북미", 
                                  showarrow = F, xref='paper', yref='paper', 
                                  xanchor = 'center')),
  p5 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "남미'", 
                                  showarrow = F, xref='paper', yref='paper', 
                                  xanchor = 'center')),
  p6 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "오세아니아", 
                                  showarrow = F, xref='paper', yref='paper', 
                                  xanchor = 'center')),
  p7 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "아프리카", 
                                  showarrow = F, xref='paper', yref='paper', 
                                  xanchor = 'center')),
  nrows = 3, 
  margin = 0.025, 
