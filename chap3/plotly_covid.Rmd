...
앞의 `plotly` 객체에서 보면 `ggplot` 객체를 정확히 전환하지는 못한다.[^3] 그러나 `ggplotly()`는 `ggplot2`패키지를 사용해서 생성한 대부분의 `ggplot` 객체를 전환할 수 있다. 게다가 `ggplot2` 패키지를 확장해서 사용하게 해주는 `ggforce`, `GGally`와 같은 확장 패키지로 생성된 객체도 변환이 가능하다는 장점이 있다. 따라서 `plotly` 객체를 다루는 방법을 잘 익히면 기존에 생성했던 `ggplot` 객체의 시각화를 재활용 할 수있게 된다. 그리고 `plotly`를 사용하다보면 느끼겠지만 `ggplot2`의 통계 요소와 분할 요소의 몇몇 요소들은 `plotly`보다는 `ggplot2`가 훨씬 편리할 떄가 있다. 이러한 경우 `ggplot2`와 `plotly`를 적절히 혼용하면 매우 좋은 결과를 얻을 수 있다.

[^3]: 앞의 예에서 범례에 나타난 오류는 ggplotly()에서 다중 범례(Multiple Legend)의 변환 과정에서 발생하는 문제로 ggplot의 범례를 제거하고 plotly의 범례를 생성함으로써 해결가능하다.
...
names(df_취업통계_2000)[10:12] <- c('졸업자수', '취업률', '취업자수')

df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수) |>
...
```{r eval = FALSE}
df_취업통계_2000 |> 
  plot_ly(x = ~졸업자수, y = ~취업자수, color = ~대계열, colors = 'Accent') |>
  layout(title = '졸업자 대비 취업자수', margin = margins)
...
```{r echo = FALSE}
p1 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, color = ~대계열, colors = 'Accent') |>
  layout(title = '졸업자 대비 취업자수')

color_vector <- c('red', 'blue', 'green', 'yellow', 'purple', 'black', 'pink')

p2 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, color = ~대계열, colors = color_vector) |>
  layout(title = '졸업자 대비 취업자수')

p3 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, color = ~대계열, colors = colorRamp(c('red', 'yellow', 'blue'))) |>
  layout(title = '졸업자 대비 취업자수')

subplot(
  p1 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "colors = 'Accent'", showarrow = F, xref='paper', yref='paper', xanchor = 'center')),
  p2 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "color = color_vector", showarrow = F, xref='paper', yref='paper', xanchor = 'center')),
  p3 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "colorRamp(c('red', 'yellow', 'blue'))", showarrow = F, xref='paper', yref='paper', xanchor = 'center')), titleY = T, margin = 0.05
) |>
...
df_취업통계_symbol |> 
  plot_ly(x = ~졸업자수, y = ~취업자수, symbol = ~symbol) |>
  layout(title = '졸업자 대비 취업자수', margin = margins)

df_취업통계_symbol |> 
  plot_ly(x = ~졸업자수, y = ~취업자수, symbol = ~factor(symbol)) |>
  layout(title = '졸업자 대비 취업자수', margin = margins)
...
p1 <- df_취업통계_symbol |> plot_ly(x = ~졸업자수, y = ~취업자수, symbol = ~symbol) |>
  layout(title = '졸업자 대비 취업자수')

p2 <- df_취업통계_symbol |> plot_ly(x = ~졸업자수, y = ~취업자수, symbol = ~factor(symbol)) |>
  layout(title = '졸업자 대비 취업자수')
...
```{r echo = FALSE}
p1 <- df_취업통계_symbol |> plot_ly(x = ~졸업자수, y = ~취업자수, symbol = 'circle-open', color = I('#1f77b4')) |>
  layout(title = '졸업자 대비 취업자수')

p2 <- df_취업통계_symbol |> plot_ly(x = ~졸업자수, y = ~취업자수, symbol = I('circle-open'), color = I('#1f77b4')) |>
  layout(title = '졸업자 대비 취업자수')
...
```{r echo = FALSE}
p1 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, stroke = 'black', color = I('white')) |>
  layout(title = '졸업자 대비 취업자수')

p2 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, stroke = I('black'), color = I('white')) |>
  layout(title = '졸업자 대비 취업자수')


subplot(
  p1 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "stroke = 'black'", showarrow = F, xref='paper', yref='paper', xanchor = 'center')),
  p2 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "stroke = I('black')", showarrow = F, xref='paper', yref='paper', xanchor = 'center')), titleY = T, margin = 0.05
) |> hide_legend() |>
  layout(title = '졸업자 대비 취업자수', margin = margins)
```
...
```{r echo = FALSE}
p1 <- df_취업통계_symbol |> plot_ly(x = ~졸업자수, y = ~취업자수, size = ~symbol, sizemode = 'diameter', alpha = 0.3, color = I('#1f77b4'))

p2 <- df_취업통계_symbol |> plot_ly(x = ~졸업자수, y = ~취업자수, size = ~symbol, sizemode = 'area', alpha = 0.3, color = I('#1f77b4'))
...
```{r echo = FALSE}
p1 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, alpha = 0.3, color = I('#1f77b4'))

p2 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, opacity = 0.3, color = I('#1f77b4'))
...
p_histogram |> add_trace(type = 'histogram', x = ~취업률, name = '취업률') |>
...
```{r}
p_histogram |> add_trace(type = 'histogram', x = ~취업률, 
...
p_histogram |> add_trace(type = 'histogram', x = ~대계열, y = ~as.character(취업률), 
...
스캐터 trace에서 약 50여개가 넘는 type을 사용하여 세부 설정이 가능하다. 이 50여개의 type 중에는 다시 세부 type을 설정하는 type이 있고 총 4단계까지 내려가는 type도 존재한다. 세부 하위 type은 `list()`를 사용하여 세부 타입을 설정할 수 있다. 이 모든 type을 다 설명할 수는 없기 때문에 주요한 type을 위주로 설명한다. 전체 type은 plotly.com에 수록되어 있다.[^5]

[^5]: <https://plotly.com/r/reference/scatter/>
...
p_line_wide |> add_trace(type = 'scatter', mode = 'markers', x = ~date, y = ~확진자_한국) |>
  layout(title = '최근 100일간 코로나19 확진자수', 
         yaxis = list(title = list(text = '확진자수')), 
         margin = margins)

p_line_wide  |> add_trace(type = 'scatter', mode = 'markers+lines', x = ~date, y = ~확진자_한국) |>
  layout(title = '최근 100일간 코로나19 확진자수', 
         yaxis = list(title = list(text = '확진자수')), 
         margin = margins)

p_line_wide  |> add_trace(type = 'scatter', mode = 'markers+text', x = ~date, y = ~확진자_한국, text = ~확진자_한국) |>
  layout(title = '최근 100일간 코로나19 확진자수', 
         yaxis = list(title = list(text = '확진자수')), 
         margin = margins)

p_line_wide  |> add_trace(type = 'scatter', mode = 'markers+lines+text', x = ~date, y = ~확진자_한국, text = ~확진자_한국) |>
  layout(title = '최근 100일간 코로나19 확진자수', 
         yaxis = list(title = list(text = '확진자수')), 
         margin = margins)
...
p_marker |>
  add_trace(type = 'scatter', mode = 'markers', x = ~졸업자수, y = ~취업자수,
...
p2 <- p_marker |>
  add_trace(type = 'scatter', mode = 'markers', x = ~졸업자수, y = ~취업자수, color = ~대계열) |>
  layout(title = '졸업자 대비 취업자수')

p3 <- p_marker |>
  add_trace(type = 'scatter', mode = 'markers', x = ~졸업자수, y = ~취업자수, marker = list(color = 'blue')) |>
  layout(title = '졸업자 대비 취업자수')

p4 <- p_marker |>
  add_trace(type = 'scatter', mode = 'markers', x = ~졸업자수, y = ~취업자수, color = I('blue')) |>
  layout(title = '졸업자 대비 취업자수')
...
`marker`의 설정에서 하나 중요한 type은 `colorbar()`이다. `color` type에 매핑되는 변수가 이산형일 경우는 각각의 카테고리에 따라 설정된 색 팔레트에 정의된 색상이 설정되지만 연속형일 경우는 연속된 색상이 설정된다. 연속된 색상에서 변수에 따른 색상이 선택되어 표시된다. 이렇게 연속된 색상의 설정에 사용되는 색 스케일이 `colorbar()`이다.
...
df_취업통계_2000 |> 
  plot_ly(x = ~졸업자수, y = ~취업자수, 
          color = ~취업률, colors = color_vector) |>
  layout(title = '졸업자 대비 취업자수', margin = margins)

df_취업통계_2000 |> 
  plot_ly(x = ~졸업자수, y = ~취업자수, 
          color = ~취업률, colors = colorRamp(c('red', 'white', 'blue'))) |>
  layout(title = '졸업자 대비 취업자수', margin = margins)

```

```{r echo = FALSE}
p1 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, color = ~취업률, colors = 'Accent')

color_vector <- c('red', 'blue', 'green', 'yellow', 'purple', 'black', 'pink')

p2 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, color = ~취업률, colors = color_vector)

p3 <- df_취업통계_2000 |> plot_ly(x = ~졸업자수, y = ~취업자수, color = ~취업률, colors = colorRamp(c('red', 'white', 'blue')))
...
p_line |> 
  add_trace(type = 'scatter', mode = 'lines', x = ~date, y = ~people_fully_vaccinated_per_hundred, 
            color = ~ iso_code, colors = 'viridis') |>
...
fig <- fig |> add_trace(type = 'scatter', mode = 'lines', x = ~date, y = ~백신접종완료자_아시아, 
                        name = '아시아')

fig |> add_trace(type = 'scatter', mode = 'lines', x = ~date, y = ~백신접종완료자_유럽, 
                 name = '유럽') |>
  layout(xaxis = list(title = list(text = NULL)), 
         yaxis = list(title = list(text = '접종완료자수')), 
         colorway = viridisLite::viridis(3))
```
...
```{r}
fig <- plot_ly() |> add_trace(data = covid19_df |> filter(continent == 'Asia'), 
...
fig |> add_trace(data = covid19_df |> filter(iso_code == 'KOR'), type = 'scatter', mode = 'lines', 
                 x = ~date, y = ~people_fully_vaccinated_per_hundred, name = '한국', 
                 line = list(color = 'red'), 
                 showlegend = T)|>
...
add_trace(p_line_wide, type = 'scatter', mode = 'text', 
  ...
  layout(title = list(text = '우리나라 접종완료자수'),
         xaxis = list(title = list(text = NULL)), 
         yaxis = list(title = '인구 100명당 접종완료자수'), 
         margin = margins)
...
subplot(
  ...
  p2 |> layout(annotations = list(x = 0.5 , y = 1.05, text = "text = ~한국", showarrow = F, xref='paper', yref='paper', xanchor = 'center')), titleY = T, margin = 0.05
) |> hide_legend() |>
  layout(title = list(text = '우리나라 접종완료자수'),
         xaxis = list(title = list(text = NULL)), 
         yaxis = list(title = '인구 100명당 접종완료자수'), 
...
p_bar |>
  ...
  layout(title = list(text = '지역별 코로나19 확진자수'),
         xaxis = list(title = list(text = '지역'), 
                      ticktext = c('한국', '아시아', '유럽', '북미', '오세아니아', '남미'), 
...
plot_ly() |> add_trace(type = 'bar', x = 1:3, y = diff(diff_sam), 
  ...
  layout(barmode = 'relative', 
         xaxis = list(ticktext = c('1월', '2월', '3월'), 
                      tickvals = c(1, 2, 3)), 
         yaxis = list(title = '증감량'), 
         title = list(text = '남미 코로나19 확진자수 증감'),
         margin = margins)
...
fig <- p_bar_mode1 |>
  add_trace(type = 'bar', x = ~iso_code, y = ~`2021-12-01`, color = I('gray30'),  
            name = '2021월 12월', text = ~`2021-12-01`, 
...
`texttemplate`은 텍스트가 표시되는 형태를 설정하는 type이다. `texttemplate`는 `hovertemplate`의 정의와 같이 사용되는데 '%{변수:변수포맷}'의 형태로 사용된다. 변수 포맷에서 사용하는 포맷은 자바 스크립트의 d3 format[^6]을 사용한다. 앞의 예에서 일반대학의 입학생수의 포맷에 천단위 콤마를 넣는다면 '%{text:,}'로 설정하고 소수점 아래 두째 자리까지 표기한다면 '%{text:2f}'로 설정한다.

[^6]: <https://github.com/d3/d3-format/tree/v1.4.5#d3-format>
...
p_box |> add_trace(type = 'box', x = ~대계열, y = ~취업률) |>
...
```{r}
p_box_group <- p_box |> add_trace(type = 'box', x = ~대계열, y = ~취업률, 
                                  color = ~과정구분)
...
```{r}
p_box_add <- p_box |> 
  add_trace(data = df_취업통계_2000 |> filter(과정구분 == '대학과정'),
            type = 'box', x = ~대계열, y = ~취업률, name = '대학')

p_box_add <- p_box_add |> 
  add_trace(data = df_취업통계_2000 |> filter(과정구분 == '전문대학과정'), 
            type = 'box', x = ~대계열, y = ~취업률, name = '전문대학')
...
```{r}
p_box_quartilemethod <- df_취업통계_2000 |> filter(대계열 == '인문계열') |> 
  head(9) |> plot_ly()
...
```{r eval = FALSE}
p_box |> add_trace(type = 'box', x = ~대계열, y = ~취업률, boxmean = TRUE) |>
  layout(title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)

p_box |> add_trace(type = 'box', x = ~대계열, y = ~취업률, boxmean = 'sd') |>
  layout(title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)
...
plot_ly() |> 
  add_trace(type= 'box', q1=c(1, 2, 3), median=c(4, 5, 6),
            ...
            sd=c(0.2, 0.4, 0.6)) |>
  layout(title = list(text = '수동으로 박스플롯 만들기'), 
         margin = margins)
```
...
p_violin |> add_trace(type = 'violin', x = ~대계열, y = ~취업률) |>
  layout(title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)
```
...
```{r}
p_violin |> add_trace(data = df_취업통계_2000 |> filter(과정구분 == '대학과정'), 
...
```{r}
p_violin |> add_trace(type = 'violin', x = ~대계열, y = ~취업률, 
                      box = list(visible = TRUE, fillcolor = 'darkred', 
                                 line = list(color = 'darkslategray'), width = 0.5
                                 )
                      ) |> 
  layout(violinmode = 'group', 
         title = list(text = '대학 계열별 취업률 분포'), 
...
p_violin |>
  add_trace(type = 'violin', x = ~대계열, y = ~취업률, 
            meanline = list(visible = TRUE, color = 'darkred', width = 1)
            ) |> 
  layout(violinmode = 'group', 
         title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)
...
p_violin |>
  add_trace(type = 'violin', x = ~대계열, y = ~취업률, 
            meanline = list(visible = TRUE, color = 'dardred', width = 1), 
            marker = list(opacity = 0.2, size = 5, color = 'indigo'),
...
p_pie <- df_취업통계_2000 |> group_by(대계열) |>
  summarise(졸업자수 = sum(졸업자수)) |> plot_ly()
...
```{r eval=FALSE}
p_pie |>
  add_trace(type = 'pie', values = ~졸업자수, 
            labels = ~대계열, textinfo = 'value') |> 
  layout(title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)

p_pie |>
  add_trace(type = 'pie', values = ~졸업자수, 
            labels = ~대계열, textinfo = 'percent') |> 
  layout(title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)

p_pie |>
  add_trace(type = 'pie', values = ~졸업자수, 
            labels = ~대계열, textinfo = 'label+percent') |> 
  layout(title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)

p_pie |>
  add_trace(type = 'pie', values = ~졸업자수, 
            labels = ~대계열, textinfo = 'label+value') |> 
  layout(title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)
...
p_pie |>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, 
            ...
            marker = list(colors = 'viridis', line = list(color = 'white', width = 2))) |> 
  layout(title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)
```
...
```{r eval=FALSE}
p_pie |>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, textinfo = 'label+percent', insidetextorientation = 'horizontal') |> 
  layout(title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)

p_pie |>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, textinfo = 'label+percent', insidetextorientation = 'radial') |> 
  layout(title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)

p_pie |>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, textinfo = 'label+percent', insidetextorientation = 'tangential') |> 
  layout(title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)

p_pie |>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, textinfo = 'label+percent', insidetextorientation = 'auto') |> 
  layout(title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)
...
```{r echo=FALSE}
p_pie |>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, textinfo = 'label+percent', insidetextorientation = 'horizontal', title = list(text = "insidetextorientation = 'horizontal'", position = 'top center', font = list(size = 15)), domain = list(x = c(0, 0.49), y = c(0.51, 1)), insidetextfont = list(size = 6)) |>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, textinfo = 'label+percent', insidetextorientation = 'radial', title = list(text = "insidetextorientation = 'radial'", position = 'top center', font = list(size = 15)), domain = list(x = c(0.51, 1), y = c(0.51, 1)), insidetextfont = list(size = 6)) |>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, textinfo = 'label+percent', insidetextorientation = 'tangential', title = list(text = "insidetextorientation = 'tangential'", position = 'top center', font = list(size = 15)), domain = list(x = c(0, 0.49), y = c(0, 0.49)), insidetextfont = list(size = 6)) |>
  add_trace(type = 'pie', values = ~졸업자수, labels = ~대계열, textinfo = 'label+percent', insidetextorientation = 'auto', title = list(text = "insidetextorientation = 'auto'", position = 'top center', font = list(size = 15)), domain = list(x = c(0.51, 1), y = c(0, 0.49)), insidetextfont = list(size = 6)) |>
  layout(showlegend = FALSE) |> 
  layout(title = list(text = '대학 계열별 취업률 분포'), 
         margin = margins)
...
`gauge`는 게이지 형태의 시각화를 통해서 단일 수치를 표현한다. 게이지의 형태는 둥근 반원 형태의 게이지를 사용하는 'angler'와 막대 형태의 게이지를 사용하는 'bullet'의 두가지가 제공된다. 이는 `shape` type을 사용하여 설정이 가능하다.
...
```{r}
p_title_marker <- df_취업통계_2000 |> plot_ly()

p_title_1 <- p_title_marker |>
  add_trace(type = 'scatter', mode = 'markers', x = ~졸업자수, 
            y = ~취업자수)
...
```{r}
p_title_1 |> 
  layout(title = list(text = "<span style = 'font-size:15pt'><span style = 'color:red;font-weight:bold;'> 졸업수</span><span style = 'font-size:10pt'> 대비</span> <span style = 'color:blue;font-weight:bold;'>취업률</span></span>", 
                      x = 0.1, xanchor = 'left', yanchor = 'top')
                    )
```
...
`plotly`에서 사용하는 색의 설정은 색 이름 설정, 16진수로 설정된 RGB값 설정, `rgb()` 함수를 사용한 RGB값의 설정 세 가지 방법이 주로 사용된다. 이외에도 색조, 채도, 명도(lightness)를 사용하는 `hsl()`을 사용하는 방법, 색조, 채도, 명도(value)를 사용하는 `hsv()`를 사용하는 방법도 있다. `plotly`에서 사용이 가능한 색 이름은 W3.org에서 제공하는 CSS 색 이름을 사용한다.[^7]

[^7]: <https://www.w3schools.com/cssref/css_colors.asp>
...
```{r}
df_annotation <- df_취업통계 |> filter(졸업자_계 < 500, 학과명 == '컴퓨터공학과') |> 
  select(과정구분, 학과명, 졸업자_계, 취업자_합계_계)
...
p_title_1 |> 
  layout(title = list(text = '<b>졸업자 대비 취업자수</b>', x = 0.1, 
         ...
         annotations = list(text = ~paste0('<b>', df_annotation$과정구분, ':', df_annotation$학과명, '</b>'), 
                            x = df_annotation$졸업자_계, 
                            y = df_annotation$취업자_합계_계, 
                            ax = 30, ay = -30), 
         margin = list(t = 50)) |>
...
p_title_1 |> 
  layout(title = list(text = '<b>졸업자 대비 취업자수</b>', x = 0.1, 
                      xanchor = 'left', yanchor = 'top', size = 15), 
         font = list(family = "나눔고딕", color = 'black', size = 10), 
         annotations = list(text = ~paste0('<b>', df_annotation$과정구분[1], ':', df_annotation$학과명[1], '</b>'), 
                            x = df_annotation$졸업자_계[1], 
                            y = df_annotation$취업자_합계_계[1], 
                            ax = 30, ay = -30, 
                            arrowhead = 3, arrowcolor = 'red', 
                            arrowsize = 1.2)
         ) |>
  add_annotations(text = ~paste0('<b>', df_annotation$과정구분[2], ':', df_annotation$학과명[2], '</b>'), 
                  x = df_annotation$졸업자_계[2], 
                  y = df_annotation$취업자_합계_계[2], 
                  ax = 30, ay = -30, 
                  arrowhead = 6, arrowcolor = 'blue', 
                  arrowsize = 0.8, 
