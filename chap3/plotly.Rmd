...
covid_stat_by_nation <- coronavirus_df %>%
  ...
  group_by(zoo::yearmon(date), type, continent_name) %>%
...
df_취업통계 |> filter(졸업자_계 < 500) |> 
  plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, name = legend_items)
...
```{r}
df_취업통계 |> filter(졸업자_계 < 500) |> 
  plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, hovertext = ~중계열)
...
df_취업통계 |> filter(졸업자_계 < 500) |> 
  plot_ly(x = ~졸업자_계, y = ~취업자_합계_계, name = ~대계열, 
          hoverinfo = 'x+y', hovertemplate = ' 졸업자:%{x}, 취업자:%{y}')
```
