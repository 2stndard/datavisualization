...
```{r}
p_marker |>
  add_markers(x = ~졸업자_계, y = ~취업자_합계_계, mode = 'markers', name = ~대계열)
...
p_marker |> add_markers(x = ~졸업자_계, y = ~취업자_합계_계, stroke = ~대계열, strokes = 'Accent', color = I('white'), name = legend_items)
...
```{r}
p_marker |>
  add_markers(x = ~졸업자_계, y = ~취업자_합계_계, mode = 'markers', name = ~대계열, showlegend = FALSE)
...
```{r}
p_marker |>
  add_markers(x = ~졸업자_계, y = ~취업자_합계_계, mode = 'markers', name = ~대계열, hovertext = ~중계열)
...
```{r}
p_marker |>
  add_markers(x = ~졸업자_계, y = ~취업자_합계_계, mode = 'markers', name = ~대계열, hoverinfo = 'x+y+name')
...
```{r}
p_marker |>
  add_markers(x = ~졸업자_계, y = ~취업자_합계_계, mode = 'markers', name = ~대계열, hoverinfo = 'x+y+text', hovertext = ~중계열)
...
```{r}
p_marker |>
  add_markers(x = ~졸업자_계, y = ~취업자_합계_계, mode = 'markers', name = ~대계열, meta = ~대계열, hoverinfo = 'x+y', hovertemplate = ' 졸업자:%{x}, 취업자:%{y}')
```
...
```{r}
p_line_4 <- df_입학자_long |> filter(지역 == '전체', 학교종류 %in% c('전문대학', '일반대학', '석사', '박사')) |> plot_ly()

add_trace(p_line_4, type = 'scatter', mode = 'lines', x = ~연도, y = ~입학생수, color = ~학교종류, linetype = ~ 학교종류)
...
```{r eval = FALSE}
p_marker |>
  add_trace(type= 'scatter', mode = 'markers', x = ~졸업자_계, y = ~취업자_합계_계, color = 'black')
```
...
```{r eval = FALSE}
p_marker |>
  add_trace(type= 'scatter', mode = 'markers', x = ~졸업자_계, y = ~취업자_합계_계, color = I('black'))

```

```{r echo = FALSE}
p1 <- p_marker |>
  add_trace(type= 'scatter', mode = 'markers', x = ~졸업자_계, y = ~취업자_합계_계, color = 'black', showlegend = FALSE)

p2 <- p_marker |>
  add_trace(type= 'scatter', mode = 'markers', x = ~졸업자_계, y = ~취업자_합계_계, color = I('black'), showlegend = FALSE)
...
```{r eval = FALSE}
p_marker |>
  add_trace(type = 'scatter', mode = 'markers', x = ~졸업자_계, y = ~취업자_합계_계, color = ~대계열, colors = 'Accent')

color_vector <- c('red', 'blue', 'green', 'yellow', 'purple', 'black', 'pink')

p_marker |>
  add_trace(type = 'scatter', mode = 'markers', x = ~졸업자_계, y = ~취업자_합계_계, color = ~대계열, colors = color_vector)

p_marker |>
  add_trace(type = 'scatter', mode = 'markers', x = ~졸업자_계, y = ~취업자_합계_계, color = ~대계열, colors = colorRamp(c('red', 'white', 'blue')))
...
`alpha`와 `opacity`는 점의 투명도를 설정하는 속성이다. `alpha`와 `opacity`는 0부터 1까지의 수치로 설정하며 0은 완전 투명으로 보이지 않고 1은 불투명한 점을 표현하는데 `alpha`는 서로 겹쳐진 부분의 투명도가 양쪽의 투명도가 더해져서 점점 불투명해지는데 반해 `opacity`는 겹쳐진 부분도 처음 설정된 투명도가 유지된다는 차이가 있다. 이 두 type은 산점도의 표현시 오버플로팅을 방지하고 색으로 구분되는 데이터 그룹간의 표현에 효과적으로 사용된다.
...
`marker`의 색 설정에서 하나 추가적인 설정은 `colorbar()`이다. `color` 매개변수에 매핑되는 변수가 이산형일 경우는 각각의 카테고리에 따라 색상이 설정되지만 연속형일 경우는 변수의 수치에 따라 색상이 변경된다. 이러한 경우 사용되는 색 스케일이 `colorbar()`이다.
...
```{r}
p_line <- df_입학자_long |> filter(지역 == '전체', 학교종류 %in% c('전문대학', '일반대학', '석사', '박사')) |> plot_ly()

add_trace(p_line, type = 'scatter', mode = 'lines', x = ~연도, y = ~입학생수, color = ~ 학교종류)
...
`dash`은 선의 형태를 설정하는 type이다. `plotly`의 
